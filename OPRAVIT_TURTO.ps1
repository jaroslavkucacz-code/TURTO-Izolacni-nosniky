...
$AppCommit = '22882c141c61f54bb1ee658030030ecd38c5c898'
$UpdaterCommit = 'c396b4d2e7c7fafc3e40e726b29456f6ed8d2f0e'
$AppSha256 = 'e9f7ebf8141201ae25d9a063737fd0466c847e4a2dbac298bf26c66c7f2ba5f8'
$UpdaterSha256 = '86ada2941c27d3cce62bf55ba4577cd39222c1df5dea2bdac7498c4bd429f21c'
    ...
    for ($attempt = 1; $attempt -le 4; $attempt++) {
        try {
            ...
            Invoke-WebRequest -UseBasicParsing -Uri $Url -OutFile $Destination -Headers @{
                'User-Agent' = 'TURTO-Recovery-3.0.9'
...
try {
    ...
    if (-not (
        (Test-Path (Join-Path $target 'app.pyw')) -or
        (Test-Path (Join-Path $target 'Spustit_program.vbs')) -or
        (Test-Path (Join-Path $target 'updater.py'))
    )) {
    ...
    # Odstranění markeru vynutí kontrolu a případnou opravu Program při prvním startu.
