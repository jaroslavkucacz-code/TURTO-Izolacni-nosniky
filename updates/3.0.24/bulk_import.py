...
from decoder_length_320 import parse_length, metres, item_length
...
class BulkImportDialog(tk.Toplevel):
    ...
    Nevyřešené položky se vloží bez domyšlených katalogových hodnot.
    ...
    def __init__(
        ...
        *,
        ...
        self._analysis_queue: queue.Queue[tuple[Any, ...]] = queue.Queue()
        self._analysis_cancel_event = threading.Event()
        self._analysis_thread: threading.Thread | None = None
        self._analysis_running = False
        self._analysis_incomplete = False
        self._analysis_previous_items: list[BulkImportItem] = []
        self._analysis_previous_headers = 0
        ...
        self.analysis_progress_text_var = tk.StringVar(value="")
        ...
        ttk.Label(
            ...
            text=(
                ...
                "Vloží se všechny položky; chybějící údaje doplníte v záložce Záměny."
            ),
            style="Muted.TLabel",
            wraplength=1300,
            justify="left",
        ...
        self.analysis_progress_frame = ttk.Frame(input_card, style="Card.TFrame")
        self.analysis_progress_frame.grid(row=1, column=0, sticky="ew", pady=(0, 8))
        self.analysis_progress_frame.columnconfigure(0, weight=1)
        ttk.Label(
        ...
        self.analysis_progress.grid(row=1, column=0, sticky="ew")
        self.analysis_cancel_button = ttk.Button(
            self.analysis_progress_frame,
            text="Storno analýzy",
            command=self._request_analysis_cancel,
        )
        self.analysis_cancel_button.grid(row=1, column=1, padx=(8, 0))
        self.analysis_cancel_button.configure(state="disabled")
        self.analysis_progress_frame.grid_remove()
        ...
        ttk.Label(input_card, text="Vlastní délky: záhlaví Pozice; Ks; Označení; Délka [m]; Poznámka, např. P001; 2; XT-…; 0,5; …\nBez záhlaví lze přidat poznámku délka=0,5 m. Hodnoty bez jednotky ve sloupci Délka jsou v metrech.",
                  style="MutedCard.TLabel", wraplength=1100, justify="left").grid(row=3, column=0, sticky="w", pady=(6,0))
        ...
        self.review_tree = ttk.Treeview(
            review_frame,
            columns=("line", "status", "position", "qty", "length", "input", "resolved", "issue"),
            show="headings",
            selectmode="extended",
        ...
        length_bar = ttk.Frame(review_card, style="Card.TFrame")
        length_bar.grid(row=2, column=0, sticky="ew", pady=(8, 0))
        self.length_var = tk.StringVar(self, value="")
        ttk.Label(length_bar, text="Délka [m]", style="Card.TLabel").grid(row=0, column=0)
        self.length_entry = ttk.Entry(length_bar, textvariable=self.length_var, width=9)
        self.length_entry.grid(row=0, column=1, padx=6)
        self.length_apply_button = ttk.Button(length_bar, text="Použít na označené", command=self._apply_length)
        self.length_apply_button.grid(row=0, column=2, padx=(0,6))
        self.length_reset_button = ttk.Button(length_bar, text="Z označení / katalogu", command=lambda: self._apply_length(reset=True))
        self.length_reset_button.grid(row=0, column=3)
        self.length_entry.bind("<Return>", lambda _e: self._apply_length())
        ...
        self.review_tree.bind("<ButtonPress-1>", self._on_review_tree_press, add="+")
    ...
    def _apply_length(self, reset=False):
        if self._analysis_running:
            return
        selected = [self._item_by_iid[i] for i in self.review_tree.selection() if i in self._item_by_iid]
        if not selected:
            messagebox.showinfo("Délka prvku", "Označte jeden nebo více řádků (Ctrl / Shift).", parent=self)
            return
        try:
            length = None if reset else parse_length(self.length_var.get())
            if length is None and not reset:
                raise ValueError("Doplňte délku, například 0,5.")
        except ValueError as exc:
            messagebox.showwarning("Neplatná délka", str(exc), parent=self)
            return
        for item in selected:
            item.element_length_mm = length
    ...
    def _review_layout_state(self) -> dict[str, Any]:
        ...
        if 'length' not in order and 'qty' in order:
            order.insert(order.index('qty') + 1, 'length')
    ...
    def _apply_review_layout(self) -> None:
        ...
        for column, (default_width, _anchor) in self._review_column_defaults.items():
            self.review_tree.column(column, width=int(state["widths"].get(column, default_width)), stretch=False)
    ...
    def _on_review_tree_press(self, event: tk.Event[Any]) -> None:
        self._review_resizing = self.review_tree.identify_region(event.x, event.y) == "separator"

    def _on_review_tree_release(self, event: tk.Event[Any]) -> None:
        if getattr(self, '_review_resizing', False):
            self._review_resizing = False
    ...
    def _set_analysis_running(self, running: bool) -> None:
        ...
        if running:
            self.analysis_progress_frame.grid()
    ...
    def _request_analysis_cancel(self) -> None:
        ...
        self.analysis_cancel_button.configure(state="disabled")
        self.analysis_progress_text_var.set(
            "Storno požadováno – dokončuji právě zpracovávaný řádek…"
        )

    def _analysis_worker(self, value: str) -> None:
        last_progress: BulkAnalysisProgress | None = None

        def on_progress(progress: BulkAnalysisProgress) -> None:
            nonlocal last_progress
            last_progress = progress
            self._analysis_queue.put(("progress", progress))

        try:
            items, skipped_headers, cancelled = analyze_bulk_text_progressive(
                self.database,
                value,
                ...
                cancel_check=self._analysis_cancel_event.is_set,
            )
        except Exception as exc:
            self._analysis_queue.put(("error", exc))
        else:
            self._analysis_queue.put(
                ("done", items, skipped_headers, cancelled, last_progress)
            )
    ...
    def _poll_analysis_queue(self) -> None:
        ...
        while True:
            ...
            kind = message[0]
            if kind == "progress":
                progress: BulkAnalysisProgress = message[1]
                total = max(1, int(progress.total))
                percent = 100.0 * min(progress.completed, total) / total
                self.analysis_progress_var.set(percent)
                designation = str(progress.designation or "").strip()
                if len(designation) > 95:
                    designation = designation[:92] + "…"
                line = f"Řádek {progress.current}/{progress.total} • {progress.phase}"
                if designation:
                    line += f" • {designation}"
                if progress.cache_hits:
                    line += f" • cache {progress.cache_hits}×"
                self.analysis_progress_text_var.set(line)
                continue

            if kind == "error":
                finished = True
                self.items = self._analysis_previous_items
                self.skipped_headers = self._analysis_previous_headers
                self._analysis_incomplete = False
                self._set_analysis_running(False)
                self.analysis_progress_text_var.set("Analýza se nezdařila.")
                self._refresh_review_tree()
                messagebox.showerror(
                    "Analýza výkazu se nezdařila",
                    str(message[1]),
                    parent=self,
                )
                continue
            ...
            if kind == "done":
                finished = True
                _kind, items, skipped_headers, cancelled, progress = message
                self.items = items
                self.skipped_headers = skipped_headers
                self._analysis_incomplete = bool(cancelled)
                self._set_analysis_running(False)

                if isinstance(progress, BulkAnalysisProgress):
                    total = max(1, int(progress.total))
                    self.analysis_progress_var.set(
                        100.0 * min(progress.completed, total) / total
                    )
                    cache_note = (
                        f" • {progress.cache_hits} opakovaných označení z cache"
                        if progress.cache_hits
                        else ""
                    )
                    if cancelled:
                        self.analysis_progress_text_var.set(
                            f"Analýza zastavena po {progress.completed}/{progress.total} řádcích"
                            f"{cache_note}. Výsledky jsou jen částečné."
                        )
                    else:
                        self.analysis_progress_var.set(100.0)
                        self.analysis_progress_text_var.set(
                            f"Dokončeno {progress.total}/{progress.total} řádků"
                            f"{cache_note}."
                        )
                else:
                    self.analysis_progress_text_var.set(
                        "Analýza zastavena." if cancelled else "Analýza dokončena."
                    )

                self._refresh_review_tree()
                if cancelled:
                    base = self.summary_var.get()
                    self.summary_var.set(base + " • ANALÝZA NEDOKONČENA")
                else:
                    self._focus_first_analysis_result()
                continue

        if self._analysis_running and not finished:
            self.after(60, self._poll_analysis_queue)

    def _focus_first_analysis_result(self) -> None:
        review_iid = next(
            (iid for iid, item in self._item_by_iid.items() if item.status == "review"),
            None,
        )
    ...
    def _analyze(self) -> None:
        ...
        if not value.strip():
            messagebox.showwarning(
                "Prázdné vložení",
                "Vložte alespoň jeden řádek z výkazu.",
                parent=self,
            )
            return

        self._analysis_previous_items = list(self.items)
        self._analysis_previous_headers = int(self.skipped_headers)
        self._analysis_incomplete = False
        self._analysis_cancel_event.clear()
        self._analysis_queue = queue.Queue()
        self.analysis_progress_var.set(0.0)
        nonempty = sum(1 for line in value.splitlines() if line.strip())
        self.analysis_progress_text_var.set(
            f"Připravuji analýzu {nonempty} neprázdných řádků…"
        )
        self._set_analysis_running(True)
        self._analysis_thread = threading.Thread(
            target=self._analysis_worker,
            args=(value,),
            name="turto-bulk-analysis",
            daemon=True,
        )
        # Finalize closed edit dialogs on the Tk thread before the worker's
        # allocations can trigger cyclic GC of their Tk variables.
        import gc
        gc.collect()
        self._analysis_thread.start()
        self.after(40, self._poll_analysis_queue)
    ...
    def _refresh_review_tree(self, preserve_selection: bool = False) -> None:
        ...
        self.insert_button_var.set(f"Vložit všechny položky ({len(self.items)})")
        self.insert_button.configure(state="normal" if self.items else "disabled")

        if self._analysis_incomplete:
            self.insert_button_var.set("Analýza nedokončena – spusťte znovu")
            self.insert_button.configure(state="disabled")
    ...
    def _insert_ready(self) -> None:
        if self._analysis_running or self._analysis_incomplete or not self.items:
            return
        self.manual_skipped_count = 0
        self.result = [
            dict(position=item.position, quantity=item.quantity, note=item.note,
                 element_length_mm=item.element_length_mm,
                 result=item.result if item.ready else None,
                 source_line=item.line_number,
                 source_text=item.source_designation or item.designation or item.raw,
                 raw=item.raw, status=item.status, message=item.message)
