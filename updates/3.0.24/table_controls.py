...
def _semantic_key(owner: Any, tree: ttk.Treeview) -> str:
    for name, value in vars(owner).items():
        if value is tree and name.endswith(("tree", "_tree")):
            return name
    # Dialog titles contain action names and release numbers. Neither belongs
    # in a preference key. Use the owning component and complete column schema.
    parent = tree.master
    while parent is not None and parent is not owner:
        for name, value in vars(parent).items():
            if value is tree:
                schema = "\0".join(str(c) for c in tree["columns"])
                return f"{type(parent).__module__}.{type(parent).__name__}.{name}:{hashlib.sha256(schema.encode()).hexdigest()[:12]}"
        parent = parent.master
    schema = "\0".join(str(c) for c in tree["columns"])
    return f"table:{hashlib.sha256(schema.encode()).hexdigest()[:20]}"
...
class TableController:
    def __init__(self, owner: Any, tree: ttk.Treeview, key: str, title: str) -> None:
        ...
        tree.bind("<ButtonPress-1>", self._start_resize, add="+")
    ...
    def _load_state(self) -> dict[str, Any]:
        ...
        raw = root.get(self.key, {})
        if not raw:
            legacy = f"{_slug(_safe_title(self.tree))}:{_slug('_'.join(self.defaults.order))}"
            candidates = [value for key, value in root.items()
                          if key.endswith(':'+_slug('_'.join(self.defaults.order)))
                          and isinstance(value, dict)
                          and set(value.get('order', [])) == set(self.defaults.order)]
            raw = root.get(legacy) or (candidates[0] if len(candidates) == 1 else {})
            if raw:
                root[self.key] = raw
        ...
        for c in self.defaults.order:
            try: widths[c] = max(42, min(1600, int(raw.get("widths", {}).get(c, self.defaults.widths[c]))))
    ...
    def apply_layout(self) -> None:
        if not self.tree.winfo_exists() or self.resizing: return
        ...
        for c in self.state["order"]:
            try: self.tree.column(c, width=int(self.state["widths"].get(c, self.defaults.widths[c])), stretch=False)
            except Exception: pass
    ...
    def reapply(self) -> None:
        ...
        # Inserting or sorting rows never changes column geometry. Reapplying
        # stored widths here raced with a separator drag and snapped it back.
        column = str(self.state.get("sort_column", ""))
    ...
    def _start_resize(self, event: tk.Event[Any]) -> None:
        self.resizing = self.tree.identify_region(event.x, event.y) == "separator"

    def _capture_widths(self, event: tk.Event[Any]) -> None:
        if not self.resizing:
            return
        self.resizing = False
        # The Treeview class binding finishes after this widget binding. The
        # pointer may also have left the heading while the user was dragging.
        self.tree.after_idle(self.capture_widths)

    def capture_widths(self) -> None:
...
def _eligible(owner: Any, tree: ttk.Treeview) -> bool:
    ...
    parent = tree.master
    while parent is not None and parent is not owner:
        # The bulk-import review has its own column manager and settings.
        if tree is getattr(parent, 'review_tree', None) and hasattr(parent, '_save_review_layout'):
            return False
        parent = parent.master
    try:
...
def finalize(owner: Any) -> None:
    """Restore preferences once all workspace builders have finished.

    Shear workspaces replace their initial column schema during construction.
    Applying settings before those builders ran restored Tk's default 200 px
    columns and stretch flags instead of the user's saved widths.
    """
    scan(owner)
    for controller in owner._turto_table_controllers.values():
        ...
        for col in columns:
            defaults.widths.setdefault(col, int(tree.column(col, 'width')))
            defaults.labels.setdefault(col, str(tree.heading(col, 'text') or col))
            defaults.anchors.setdefault(col, str(tree.column(col, 'anchor')))
        defaults.order = columns
        controller.state = controller._load_state()
        controller.apply_layout()
        controller._install_headings()
...
def _schedule_reapply(owner: Any) -> None:
    if getattr(owner, '_turto_table_reapply_job', None): return
    def flush():
        owner._turto_table_reapply_job = None
        reapply(owner)
    owner._turto_table_reapply_job = owner.after_idle(flush)
...
def install(owner: Any) -> None:
    ...
    def mapped(event: Any = None) -> None:
        # Mapping hundreds of HIT Entry/Label widgets must not schedule hundreds
        # of full-window walks. Only a newly mapped Treeview needs a controller.
        tree = getattr(event, 'widget', None)
        if not isinstance(tree, ttk.Treeview): return
        registry = owner._turto_table_controllers
        token = str(tree)
        if token in registry or not _eligible(owner, tree): return
        key = _semantic_key(owner, tree)
        registry[token] = TableController(owner, tree, key, _safe_title(tree) or key)
