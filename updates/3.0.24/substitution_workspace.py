...
from substitution_pdf import ensure_vector_pdf_backend, write_substitution_pdf
from ui_utils import place_dialog_on_parent


SUBSTITUTION_MODULE_VERSION = "1.1.15"
...
# blokovat automatickou záměnu, zejména potřebný přechod 300 → 330 mm.
...
_STANDARD_LENGTHS: tuple[tuple[int, int], ...] = ((100, 1000), (50, 500), (33, 330), (25, 250))
_LENGTH_KEYS_MM = (
...
def _length_is_allowed(source_length_mm: int, target_length_mm: int) -> bool:
    ...
    HIT délky 330 mm. Únosnost se i v tomto případě kontroluje na celý prvek.
    """
    source = int(source_length_mm)
    target = int(target_length_mm)
    return target <= source or (source == 300 and target == 330)
...
def _length_relation_text(source_length_mm: int, target_length_mm: int) -> str:
    ...
    if source == target:
        return "shodná délka"
    if source == 300 and target == 330:
        return "povolená výjimka 300→330 mm"
    if target < source:
...
def _detect_source_geometry(combined: str) -> tuple[str, int | None, str, list[str]]:
    """Vrátí zdrojovou geometrii, rozměr a odpovídající geometrii HIT."""
    ...
    if not target_variant:
        warnings.append(
            f"geometrie {source_variant}{dimension} zatím nemá ověřené automatické přiřazení k HIT"
        )
    return source_variant, dimension, target_variant, warnings


def _source_geometry(
    row: dict[str, Any],
    source_text: str,
) -> tuple[str, int | None, str, list[str], bool, str]:
    """Geometrie zdroje s rozlišením automatického odhadu a potvrzení uživatelem.

    Automaticky rozpoznané OU/OD je použitelné pro předběžný návrh, ale zůstává
    ve stavu KONTROLA. Teprve explicitní potvrzení nebo ruční upřesnění odstraní
    upozornění a dovolí výsledku přejít do stavu VYHOVUJE.
    """
    note = str(row.get("note", ""))
    combined = f"{source_text} | {note}"
    source_variant, detected_bx, detected_target, detected_warnings = _detect_source_geometry(combined)

    mapping = row.get("mapping") if isinstance(row.get("mapping"), dict) else {}
    overrides = mapping.get("source_overrides") if isinstance(mapping.get("source_overrides"), dict) else {}
    mode = str(overrides.get("geometry_mode", "auto") or "auto").strip().lower()
    manual_variant = str(overrides.get("geometry_variant", "") or "").strip().upper()
    manual_bx = _parse_length_mm(overrides.get("geometry_bx_mm"), "mm")
    confirmed = bool(overrides.get("geometry_confirmed", False))
    origin = str(overrides.get("geometry_origin", "") or "").strip().lower()

    if mode == "none":
        return "", None, "bez OU/OD • potvrzeno uživatelem", [], True, "manual_none"

    if mode == "manual" or (confirmed and manual_variant):
        if manual_variant not in {"OU", "OD"} or not manual_bx:
            return (
                "",
                None,
                "neplatné ruční upřesnění geometrie",
                ["ruční geometrie musí obsahovat typ OU nebo OD a kladný rozměr bx"],
                False,
                "invalid",
            )
        same_as_detected = (
            detected_target == manual_variant
            and detected_bx is not None
            and int(detected_bx) == int(manual_bx)
        )
        if source_variant and detected_bx is not None:
            display = f"{source_variant}{detected_bx} → {manual_variant}{manual_bx}"
        else:
            display = f"{manual_variant}{manual_bx}"
        if origin == "confirmed_auto" or same_as_detected:
            display += " • potvrzeno uživatelem"
            resolved_origin = "confirmed_auto"
        else:
            display += " • ručně upřesněno"
            resolved_origin = "manual"
        return manual_variant, int(manual_bx), display, [], True, resolved_origin

    if source_variant and detected_bx is not None:
        display = f"{source_variant}{detected_bx}"
        if detected_target:
            display += f" → {detected_target}{detected_bx} • čeká na potvrzení"
            warnings = [
                *detected_warnings,
                (
                    f"geometrie {detected_target}{detected_bx} byla rozpoznána automaticky; "
                    "potvrďte ji nebo upravte v dialogu Upřesnit zdroj / geometrii"
                ),
            ]
            return detected_target, int(detected_bx), display, list(dict.fromkeys(warnings)), False, "automatic"
        return "", int(detected_bx), display, detected_warnings, False, "unsupported"
...
def source_metadata(row: dict[str, Any]) -> dict[str, Any]:
    ...
    geometry_target, geometry_bx, geometry_display, geometry_warnings, geometry_confirmed, geometry_origin = _source_geometry(row, source_text)
    ...
    if not length_mm:
        ...
        errors.append(f"pro délku zdroje {length_mm} mm neexistuje přípustná standardní délka HIT; u zdroje 300 mm je povolen také HIT 330 mm")
    if compression_category == "unknown":
        errors.append("není jednoznačné, zda je původní prvek s tlakovými ložisky, nebo bez nich")
    if geometry_origin == "invalid":
        errors.append("ruční upřesnění geometrie OU/OD je neplatné")
...
def design_targets(
    ...
    *,
    ...
    if not length_options:
        ...
        return [], [f"není dostupná přípustná délka HIT pro zdroj {source_length} mm (standardně shodná/kratší; u zdroje 300 mm také 330 mm; povoleno: {allowed_text or 'nic'})"]
...
def calculation(
    ...
    geometry = str(metadata.get("geometry_display", "")).strip()
    if geometry:
        parts.append(f"geometrie: {geometry}")
    return " | ".join(parts)
...
class SubstitutionSourceDialog(tk.Toplevel):
    """Jednotný dialog pro potvrzení a ruční upřesnění vstupů záměny."""

    GEOMETRY_OPTIONS = ("Automaticky", "Bez OU/OD", "OU", "OD")
    COMPRESSION_OPTIONS = ("Automaticky", "S tlakovými ložisky", "Bez tlakových ložisek")

    def __init__(
        self,
        parent: "SubstitutionWorkspaceMixin",
        *,
        source_text: str,
        source_length_mm: int,
        compression_category: str,
        series: str,
        detected_variant: str,
        detected_bx: int | None,
        ...
        self.series = str(series or "")
        self.detected_variant = str(detected_variant or "").upper()
        self.detected_bx = int(detected_bx) if detected_bx else None

        self.title("Upřesnit zdroj a geometrii záměny")
        ...
        self.length_var = tk.StringVar(value=str(source_length_mm or 1000))
        self.compression_var = tk.StringVar(
            value={
                "bearing": "S tlakovými ložisky",
                "without": "Bez tlakových ložisek",
            }.get(str(compression_category), "Automaticky")
        )

        override_mode = str(overrides.get("geometry_mode", "auto") or "auto").lower()
        override_variant = str(overrides.get("geometry_variant", "") or "").upper()
        override_bx = _parse_length_mm(overrides.get("geometry_bx_mm"), "mm")
        if override_mode == "none":
            initial_geometry = "Bez OU/OD"
        elif override_mode == "manual" and override_variant in {"OU", "OD"}:
            initial_geometry = override_variant
        elif self.detected_variant in {"OU", "OD"}:
            # Automatický návrh je předvyplněný. Uložením jej uživatel výslovně potvrdí.
            initial_geometry = self.detected_variant
        else:
            initial_geometry = "Automaticky"
        self.geometry_var = tk.StringVar(value=initial_geometry)
        self.bx_var = tk.StringVar(value=str(override_bx or self.detected_bx or ""))

        outer = ttk.Frame(self, style="App.TFrame", padding=20)
        outer.pack(fill="both", expand=True)
        outer.columnconfigure(1, weight=1)

        ttk.Label(outer, text="Upřesnění záměny za HIT", style="DialogTitle.TLabel").grid(
            row=0, column=0, columnspan=2, sticky="w"
        )
        ttk.Label(
            outer,
            text=(
                "Automaticky rozpoznanou geometrii lze jednoduše potvrdit: ponechte předvyplněné "
                "OU/OD a bx a stiskněte „Uložit a přepočítat“. Hodnotu lze před uložením změnit."
            ),
            style="Muted.TLabel",
            wraplength=730,
            justify="left",
        ...
        ttk.Label(outer, text="Původní označení").grid(row=2, column=0, sticky="w", padx=(0, 14), pady=5)
        source_entry = ttk.Entry(outer, textvariable=self.source_var)
        source_entry.grid(row=2, column=1, sticky="ew", pady=5)

        ttk.Label(outer, text="Délka zdrojového prvku [mm]").grid(row=3, column=0, sticky="w", padx=(0, 14), pady=5)
        ttk.Entry(outer, textvariable=self.length_var, width=14).grid(row=3, column=1, sticky="w", pady=5)

        ttk.Label(outer, text="Tlakový přenos").grid(row=4, column=0, sticky="w", padx=(0, 14), pady=5)
        ttk.Combobox(
            outer,
            textvariable=self.compression_var,
            values=self.COMPRESSION_OPTIONS,
            state="readonly",
            width=28,
        ).grid(row=4, column=1, sticky="w", pady=5)

        geometry_card = ttk.Frame(outer, style="Card.TFrame", padding=12)
        geometry_card.grid(row=5, column=0, columnspan=2, sticky="ew", pady=(14, 0))
        geometry_card.columnconfigure(1, weight=1)

        ttk.Label(geometry_card, text="Geometrie výsledného HIT", style="Section.TLabel").grid(
            row=0, column=0, columnspan=4, sticky="w"
        )
        auto_text = (
            f"Automaticky rozpoznáno: {detected_display}"
            if detected_display
            else "V označení nebyla automaticky rozpoznána geometrie OU/OD."
        )
        ttk.Label(
            geometry_card,
            text=auto_text,
            ...
            wraplength=680,
            justify="left",
        ).grid(row=1, column=0, columnspan=4, sticky="ew", pady=(3, 9))

        ttk.Label(geometry_card, text="Provedení", style="Card.TLabel").grid(row=2, column=0, sticky="w")
        self.geometry_combo = ttk.Combobox(
            geometry_card,
            textvariable=self.geometry_var,
            values=self.GEOMETRY_OPTIONS,
            state="readonly",
            width=20,
        )
        self.geometry_combo.grid(row=2, column=1, sticky="w", padx=(10, 22))
        self.geometry_combo.bind("<<ComboboxSelected>>", self._geometry_changed)

        ttk.Label(geometry_card, text="bx [mm]", style="Card.TLabel").grid(row=2, column=2, sticky="w")
        self.bx_entry = ttk.Entry(geometry_card, textvariable=self.bx_var, width=12)
        self.bx_entry.grid(row=2, column=3, sticky="w", padx=(8, 0))

        maximum = 290 if self.series == "SP" else 330
        series_text = f"HIT-{self.series}" if self.series else "HIT"
        self.geometry_hint_var = tk.StringVar(
            value=f"Standardní rozsah bx pro {series_text}: 175–{maximum} mm."
        )
        ttk.Label(
            geometry_card,
            textvariable=self.geometry_hint_var,
            style="MutedCard.TLabel",
        ...
        source_entry.selection_range(0, "end")

    def _geometry_changed(self, _event: Any = None) -> None:
        active = self.geometry_var.get() in {"OU", "OD"}
        self.bx_entry.configure(state="normal" if active else "disabled")
        if active and not self.bx_var.get().strip() and self.detected_bx:
            self.bx_var.set(str(self.detected_bx))

    def _submit(self) -> None:
        source_text = self.source_var.get().strip()
        if not source_text:
            messagebox.showwarning("Zdrojový typ", "Původní označení nesmí být prázdné.", parent=self)
            return
        try:
            length = int(self.length_var.get().strip())
        except ValueError:
            messagebox.showwarning("Délka", "Délka zdrojového prvku musí být celé číslo v mm.", parent=self)
            return
        if not 1 <= length <= 10000:
            messagebox.showwarning("Délka", "Délka musí být v rozsahu 1–10 000 mm.", parent=self)
            return

        compression = {
            "Automaticky": "",
            "S tlakovými ložisky": "bearing",
            "Bez tlakových ložisek": "without",
        }.get(self.compression_var.get())
        if compression is None:
            messagebox.showwarning("Tlakový přenos", "Zvolte platný tlakový přenos.", parent=self)
            return

        selected = self.geometry_var.get()
        result: dict[str, Any] = {
            "source_text": source_text,
            "source_length_mm": length,
            "compression_category": compression,
        }
        if selected == "Automaticky":
            result.update(
                geometry_mode="auto",
                geometry_variant="",
                geometry_bx_mm=None,
                geometry_confirmed=False,
                geometry_origin="automatic",
            )
        elif selected == "Bez OU/OD":
            result.update(
                geometry_mode="none",
                geometry_variant="",
                geometry_bx_mm=None,
                geometry_confirmed=True,
                geometry_origin="manual_none",
            )
        else:
            try:
                bx = int(self.bx_var.get().strip())
            except ValueError:
                messagebox.showwarning("Geometrie", "Pro OU/OD zadejte celé číslo bx v mm.", parent=self)
                return
            maximum = 290 if self.series == "SP" else 330
            if not 175 <= bx <= maximum:
                messagebox.showwarning(
                    "Geometrie",
                    f"bx musí být pro HIT-{self.series or '?'} v rozsahu 175–{maximum} mm.",
                    parent=self,
                )
                return
            origin = (
                "confirmed_auto"
                if selected == self.detected_variant and self.detected_bx == bx
                else "manual"
            )
            result.update(
                geometry_mode="manual",
                geometry_variant=selected,
                geometry_bx_mm=bx,
                geometry_confirmed=True,
...
class SubstitutionWorkspaceMixin:
    ...
    def _build_substitution_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(
            ...
            text=(
                ...
                "Standardně se navrhuje nejbližší shodná nebo kratší délka HIT; u zdroje 300 mm je přípustný také HIT 330 mm, pokud staticky vyhoví. "
                ...
                "Automaticky rozpoznané OU/OD se předvyplní, ale pro čistý výstup VYHOVUJE je potřeba geometrii potvrdit nebo ručně upravit."
            ),
            style="MutedCard.TLabel",
        ...
        ttk.Button(bar, text="Upřesnit zdroj / geometrii…", command=self.edit_substitution_source).grid(row=0, column=3, padx=(7, 0))
        ttk.Button(bar, text="✓ Potvrdit geometrii", command=self.confirm_substitution_geometry).grid(row=0, column=4, padx=(7, 0))
        ttk.Button(bar, text="Vybrat variantu…", command=self.choose_substitution_variant).grid(row=0, column=5, padx=(7, 0))
        ttk.Button(bar, text="Kopírovat tabulku", command=self.copy_substitution_table).grid(row=0, column=6, padx=(7, 0))
        ttk.Button(bar, text="Export Excel", command=self.export_substitution_xlsx).grid(row=0, column=7, padx=(7, 0))
        ttk.Button(bar, text="Export PDF", style="Accent.TButton", command=self.export_substitution_pdf).grid(row=0, column=8, padx=(7, 0))
        review_bar = ttk.Frame(bar, style="App.TFrame")
        ...
        ttk.Label(
            ...
            text=(
                ...
                "HIT standardně nesmí být delší než zdroj; výjimkou je zdroj 300 mm, který lze nahradit HIT 330 mm. Jinak se upřednostní nejbližší shodná nebo kratší vyhovující délka. "
                "Pro automatickou záměnu se vždy prověří délky 1000, 500, 330 a 250 mm; volby délek v ručním návrhu HIT se zde nepoužívají. "
    ...
    def _payload(self, row: dict[str, Any]) -> tuple[dict[str, Any], str]:
        ...
        live_meta = source_metadata(row)
        meta = dict(live_meta)
        stored_meta = mapping.get("source_meta") if isinstance(mapping.get("source_meta"), dict) else {}
        if stored_meta:
            meta = {**live_meta, **stored_meta}
            for key in (
                "geometry_target", "geometry_bx_mm", "geometry_display",
                "geometry_confirmed", "geometry_origin", "warnings", "errors",
            ):
                meta[key] = live_meta.get(key)
        target = selected_target(mapping)
        reviewed = review_state(row, live_meta, action_warnings)
    ...
    def _reset_substitution_mapping(self, row: dict[str, Any], overrides: dict[str, Any]) -> None:
        row["mapping"] = {
            "status": "not_run",
            "targets": [],
            ...
            "source_overrides": dict(overrides),
        }

    def confirm_substitution_geometry(self) -> None:
        selected = list(self.sub_tree.selection()) if hasattr(self, "sub_tree") else []
        if not selected:
            messagebox.showinfo("Potvrzení geometrie", "Vyberte alespoň jeden řádek.", parent=self)
            return
        ...
        for row_id in selected:
            ...
            if (
                str(meta.get("geometry_origin", "")) == "automatic"
                and str(meta.get("geometry_target", "")) in {"OU", "OD"}
                and int(meta.get("geometry_bx_mm") or 0) > 0
                and not bool(meta.get("geometry_confirmed"))
            ):
        ...
        if not eligible:
            messagebox.showinfo(
                "Potvrzení geometrie",
                "Ve vybraných řádcích není žádná nepotvrzená automaticky rozpoznaná geometrie OU/OD.",
                parent=self,
            )
            return
        ...
        if len(eligible) == 1:
            row, meta = eligible[0]
            question = (
                f"Potvrdit pro pozici {row.get('position', '')} geometrii "
                f"{meta.get('geometry_target')}{meta.get('geometry_bx_mm')}?"
            )
        else:
            preview = ", ".join(
                f"{row.get('position', '')}: {meta.get('geometry_target')}{meta.get('geometry_bx_mm')}"
                for row, meta in eligible[:8]
            )
            if len(eligible) > 8:
                preview += f", … +{len(eligible) - 8}"
            question = f"Potvrdit automaticky rozpoznanou geometrii u {len(eligible)} řádků?\n\n{preview}"
        if not messagebox.askyesno("Potvrdit geometrii", question, parent=self):
            return
        ...
        for row, meta in eligible:
            ...
            overrides.update(
                {
                    "geometry_mode": "manual",
                    "geometry_variant": str(meta.get("geometry_target")),
                    "geometry_bx_mm": int(meta.get("geometry_bx_mm")),
                    "geometry_confirmed": True,
                    "geometry_origin": "confirmed_auto",
                }
            )
            self._reset_substitution_mapping(row, overrides)
            confirmed_ids.append(str(row.get("id")))
        ...
        self.refresh_substitution_tree()
        if confirmed_ids:
            self.sub_tree.selection_set([row_id for row_id in confirmed_ids if self.sub_tree.exists(row_id)])
        self.design_substitutions(True)
        self.sub_status_var.set(
            f"Potvrzena geometrie u {len(confirmed_ids)} řádků a návrh byl znovu přepočítán."
        )
    ...
    def edit_substitution_source(self) -> None:
        ...
        meta = source_metadata(row)
        source_text = str(row.get("source_text") or snapshot.get("designation", ""))
        length = int(overrides.get("source_length_mm") or meta.get("source_length_mm") or 1000)
        compression = str(overrides.get("compression_category") or meta.get("source_compression_category") or "unknown")

        dialog = SubstitutionSourceDialog(
            self,
            source_text=source_text,
            source_length_mm=length,
            compression_category=compression,
            series=str(meta.get("target_series", "")),
            detected_variant=str(meta.get("geometry_target", "")),
            detected_bx=int(meta.get("geometry_bx_mm") or 0) or None,
            detected_display=str(meta.get("geometry_display", "")),
            overrides=overrides,
        )
        ...
        overrides["source_length_mm"] = int(result["source_length_mm"])
        category = str(result.get("compression_category", ""))
        if category:
            overrides["compression_category"] = category
        else:
            overrides.pop("compression_category", None)

        overrides["geometry_mode"] = str(result.get("geometry_mode", "auto"))
        variant = str(result.get("geometry_variant", ""))
        bx = result.get("geometry_bx_mm")
        if variant:
            overrides["geometry_variant"] = variant
        else:
            overrides.pop("geometry_variant", None)
        if bx:
            overrides["geometry_bx_mm"] = int(bx)
        else:
            overrides.pop("geometry_bx_mm", None)
        overrides["geometry_confirmed"] = bool(result.get("geometry_confirmed", False))
        overrides["geometry_origin"] = str(result.get("geometry_origin", ""))

        row["source_text"] = str(result["source_text"])
        self._reset_substitution_mapping(row, overrides)
        self.project.touch()
        if hasattr(self, "mark_project_dirty"):
            self.mark_project_dirty()
        self.refresh_substitution_tree()
        if self.sub_tree.exists(str(row.get("id"))):
            self.sub_tree.selection_set(str(row.get("id")))
            self.sub_tree.focus(str(row.get("id")))
        if hasattr(self, "refresh_project_tree"):
            self.refresh_project_tree(select_ids=[str(row.get("id"))])

        self.design_substitutions(True)
        if str(result.get("geometry_mode")) == "manual":
            self.sub_status_var.set(
                f"Geometrie {result.get('geometry_variant')}{result.get('geometry_bx_mm')} byla potvrzena a návrh přepočítán."
            )
        elif str(result.get("geometry_mode")) == "none":
            self.sub_status_var.set("Provedení bez OU/OD bylo potvrzeno a návrh přepočítán.")
        else:
            self.sub_status_var.set(
                "Zdroj byl uložen a přepočítán; automaticky rozpoznaná geometrie zůstává do potvrzení ve stavu KONTROLA."
            )
    ...
    def choose_substitution_variant(self) -> None:
        ...
        if not target:
            return
        if str(mapping.get("selected_target_id")) != str(target.get("id")):
        ...
        mapping["status"] = review_state(row, source_metadata(row), _warnings)["status"]
        self.project.touch()
        if hasattr(self, "mark_project_dirty"):
            self.mark_project_dirty()
        self.refresh_substitution_tree()
    ...
    def confirm_substitution_choice(self) -> None:
        selected = list(self.sub_tree.selection()) if hasattr(self, "sub_tree") else []
        if len(selected) != 1:
    ...
    def design_substitutions(self, selected_only: bool) -> None:
        ...
        database = self._settings() if any(not infill.dimensions(row) for row in rows) else None
        if database is None and any(not infill.dimensions(row) for row in rows):
            return
        allowed_lengths = set(SUBSTITUTION_LENGTH_CODES)
        ok = review = failed = skipped = fillers = 0
        for row in rows:
            if infill.dimensions(row):
                infill.assign(row)
                fillers += 1
                continue
            ...
            chosen_id = str(previous_mapping.get("selected_target_id") or "")
            ...
            acceptance = previous_mapping.get("substitution_acceptance")
        ...
        self.sub_status_var.set(f"Navrženo {ok} • ke kontrole {review} • nelze {failed} • již HIT {skipped} • mezivýplně {fillers}")
    ...
    def _pdf_report_rows(self) -> list[dict[str, Any]]:
        ...
        for row in self.project.rows:
            if selected and str(row.get("id")) not in selected:
                continue
            if infill.dimensions(row):
                report.append(infill.report_row(self, row))
                continue
    ...
    def export_substitution_pdf(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            parent=self,
            title="Uložit vektorový výpočet záměn",
        ...
        if not path:
            return
        try:
            self.sub_status_var.set("Připravuji vektorový PDF výstup…")
            self.update_idletasks()
            ensure_vector_pdf_backend()
            writer = write_substitution_pdf
            if any(row.get('non_structural') for row in rows):
                from pdf_data_304 import iso_substitution_rows
                from hit_pdf import write_hit_proposal_pdf
                selected = set(self.sub_tree.selection())
                rows = [report for row, report in zip(self.project.rows, iso_substitution_rows(self))
                        if not selected or str(row.get('id')) in selected]
                writer = write_hit_proposal_pdf
            target = writer(
                Path(path),
            ...
            messagebox.showerror("Export vektorového PDF se nezdařil", str(exc), parent=self)
            return
        self.sub_status_var.set(f"Vektorový PDF výpočet exportován: {target.name}")
        try:
