...
from decoder_length_320 import parse_length, metres, effective_length, manual_length, set_length, capacity_length_issue
...
class RowMetaDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        *,
        ...
        designation: str,
        colors: dict[str, str],
        element_length_mm: int | None = None,
    ) -> None:
        ...
        self.length_var = tk.StringVar(value=metres(element_length_mm))
        ...
        ttk.Label(outer, text="Délka jednoho prvku [m]").grid(row=5, column=0, sticky="w", pady=5)
        self.length_entry = ttk.Entry(outer, textvariable=self.length_var, width=12)
        self.length_entry.grid(row=5, column=1, sticky="w", pady=5)
        ttk.Label(outer, text="Např. 0,5 nebo 0,75. Prázdné = z označení / katalogu.", style="Muted.TLabel").grid(row=6, column=0, columnspan=2, sticky="w")
    ...
    def _submit(self) -> None:
        ...
        try:
            ...
            length = parse_length(self.length_var.get())
        except ValueError as exc:
            messagebox.showwarning("Neplatné údaje řádku", str(exc), parent=self)
            return
        self.result = {
            "position": position,
            "quantity": quantity,
            "note": self.note_var.get().strip(),
            "element_length_mm": length,
        }
        self.destroy()
...
class ProjectWorkspaceMixin:
    ...
    def _project_layout_state(self) -> dict[str, Any]:
        ...
        if 'element_length' in ids and 'element_length' not in order and 'quantity' in order:
            order.insert(order.index('quantity') + 1, 'element_length')
    ...
    def _apply_project_column_layout(self) -> None:
        ...
        for column, _label, default_width, _anchor, _stretch in self.PROJECT_COLUMNS:
            ...
            self.project_tree.column(column, width=width, stretch=False)
    ...
    def _on_project_tree_press(self, event: tk.Event[Any]) -> None:
        self._project_resizing = self.project_tree.identify_region(event.x, event.y) == "separator"

    def _on_project_tree_release(self, event: tk.Event[Any]) -> None:
        if getattr(self, '_project_resizing', False):
            self._project_resizing = False
    ...
    def _build_project_tab(self, parent: ttk.Frame) -> None:
        ...
        self.project_quick_length_var = tk.StringVar(self, value="")
        length_bar = ttk.Frame(quick_card, style="Card.TFrame")
        length_bar.grid(row=3, column=0, columnspan=9, sticky="ew", pady=(6, 0))
        ttk.Label(length_bar, text="Délka prvku [m]", style="Card.TLabel").pack(side="left")
        self.project_quick_length_entry = ttk.Entry(length_bar, textvariable=self.project_quick_length_var, width=10)
        self.project_quick_length_entry.pack(side="left", padx=(6, 10))
        self.project_quick_length_entry.bind("<Return>", self.quick_add_project_row)
        ttk.Label(length_bar, text="Např. 0,5; prázdné = z označení / katalogu.", style="MutedCard.TLabel").pack(side="left")
        ...
        self.project_tree.bind("<ButtonPress-1>", self._on_project_tree_press, add="+")
    ...
    def _show_project_context_menu(self, event: tk.Event[Any]) -> str | None:
        if self.project_tree.identify_region(event.x, event.y) == "heading":
    ...
    def _row_payload(self, row: dict[str, Any], order: int) -> tuple[dict[str, Any], str]:
        ...
        payload = {
            ...
            "position": str(row.get("position", "")),
            "quantity": int(row.get("quantity", 1)),
            "element_length": metres(effective_length(row)),
        ...
        if issue:
            payload["status"] = "Ověřit délku"
            payload["note"] = " • ".join(x for x in (payload["note"], issue) if x)
            status_code = "changed"
    ...
    def sort_project_table(self, column: str, reverse: bool | None = None) -> None:
    ...
    def quick_add_project_row(self, _event: tk.Event[Any] | None = None) -> str | None:
        ...
        try:
            ...
            length = parse_length(self.project_quick_length_var.get())
        ...
        self.project_quick_length_var.set("")
    ...
    def bulk_paste_project_rows(self) -> None:
        ...
        for imported in dialog.result:
            ...
            if not isinstance(result, QueryResult):
                from source_completion import unresolved_row
                row = unresolved_row(imported)
                self.project.add(row)
            ...
            row = create_project_row(
                ...
                element_length_mm=imported.get("element_length_mm"),
            )
            self.project.add(row)
            added_ids.append(str(row["id"]))
        ...
        status = f"Hromadně vloženo {len(added_ids)} řádků. Neúplné položky doplňte v Záměnách."
        if manual_count:
    ...
    def add_or_update_project_from_lookup(self) -> None:
        ...
        dialog = RowMetaDialog(
            ...
            colors=self.colors,
            element_length_mm=manual_length(existing) if existing else None,
        )
        self.wait_window(dialog)
        if dialog.result is None:
            return
    ...
    def edit_selected_project_metadata(self) -> None:
        ...
        dialog = RowMetaDialog(
            ...
            colors=self.colors,
            element_length_mm=effective_length(row),
        )
        self.wait_window(dialog)
        if dialog.result is None:
            return
        values = dict(dialog.result)
        length = values.pop("element_length_mm")
        # Invalidate before changing note, which can also contain old length metadata.
        before = effective_length(row)
        row.update(values)
        set_length(row, length)
        if effective_length(row) != before:
            self._reset_substitution_mapping(row, row["mapping"]["source_overrides"])
        self.project.touch()
        self.mark_project_dirty()
        self.refresh_project_tree(select_ids=[str(row["id"])])
        if hasattr(self, "refresh_substitution_tree"):
