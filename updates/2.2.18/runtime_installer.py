...
"""TURTO 2.2.18 recovery installer; actions.sqlite3 is never modified."""
...
PAYLOADS = {
    "app_runtime.pyw": (
        "e6d26c38a6c23ea82b554ceb8e68b7ca79a6e603",
        "updates/2.2.18/app_runtime.pyw",
        "92522066a84089900cc151a456229902e4a6670eb8deb85fb6319b66799d65c3",
    ),
    "hit_workspace.py": (
...
def _ensure_previous_runtime(root: Path, program: Path, temp: Path) -> None:
    ...
    base.write_bytes(
        _download(BASE_COMMIT, BASE_PATH, BASE_SHA256, "TURTO-2.2.18-base")
    )
    ...
    if not (_revision(program, PREVIOUS_REVISION, "2.2.16") and _complete(program, PREVIOUS_REQUIRED)):
        raise RuntimeError("Ověřený runtime 2.2.16 se nepodařilo kompletně obnovit.")
...
def _apply_incremental_overlay(program: Path, temp: Path) -> None:
    ...
    for local, (commit, remote, expected) in PAYLOADS.items():
        ...
        source.write_bytes(_download(commit, remote, expected, "TURTO-2.2.18-runtime"))
        staged[local] = source
...
def install_runtime(root: Path | str) -> None:
    ...
    if _revision(program, PROGRAM_REVISION, "2.2.18") and _complete(program, CURRENT_REQUIRED):
        return

    with tempfile.TemporaryDirectory(prefix="turto_2218_") as temp_name:
        temp = Path(temp_name)
        _ensure_previous_runtime(root, program, temp)
        _apply_incremental_overlay(program, temp)

        current_marker = program / PROGRAM_REVISION
        current_marker.write_text("2.2.18", encoding="utf-8")
        ...
        if not _complete(program, CURRENT_REQUIRED):
            raise RuntimeError("Runtime 2.2.18 není po opravě kompletní.")
