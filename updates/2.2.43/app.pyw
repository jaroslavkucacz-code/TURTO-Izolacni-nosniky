...
"""TURTO 2.2.43 – exact Schöck Isokorb XT schedule analysis."""
import hashlib
import importlib
...
INSTALLER_COMMIT = "b194cfd7c25937ea25317dc236b85ba0fc06be26"
INSTALLER_SHA256 = "8f1a570370c7e47a68641d916b33a5d17007b6859983c32c902c0f714451f5ed"
RUNTIME_LAYOUT = "27"

ROOT = Path(__file__).resolve().parent
PROGRAM = ROOT / "Program"
ROOT_MARKER = ROOT / ".turto_runtime_current.ok"
PROGRAM_MARKER = PROGRAM / ".turto_runtime_2_2_43.ok"
...
def _download_installer() -> Path:
    url = f"https://raw.githubusercontent.com/{REPOSITORY}/{INSTALLER_COMMIT}/updates/2.2.43/runtime_installer.py"
    folder = Path(tempfile.mkdtemp(prefix="turto_2243_boot_"))
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(
                ...
                headers={"User-Agent":"TURTO-2.2.43-Bootstrap","Cache-Control":"no-cache, no-store","Pragma":"no-cache"},
            )
    ...
    raise RuntimeError(f"Nelze stáhnout runtime installer 2.2.43.\n{last}")
...
def _repair_runtime() -> None:
    ...
    try:
        ...
        if not PROGRAM_MARKER.is_file():
            raise RuntimeError("Opravený runtime nemá značku verze 2.2.43.")
...
def selftest() -> None:
    ...
    assert "app_runtime_242.pyw" in CRITICAL_PROGRAM_SHA256
