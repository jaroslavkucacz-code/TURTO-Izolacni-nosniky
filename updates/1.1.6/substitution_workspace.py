...
def _variant_overview(targets: list[dict[str, Any]]) -> str:
...
class SubstitutionVariantsDialog(tk.Toplevel):
    ...
    def _populate(self) -> None:
        ...
        selected_iid = ""
        for index, target in enumerate(displayed):
            iid = f"variant_{index}"
            self._target_by_iid[iid] = target
            target_id = str(target.get("id"))
            diameter = _target_wire_diameter(target)
            is_current = target_id == self._current_id
            is_best = diameter is not None and diameter not in first_by_diameter
        ...
        if not selected_iid and children:
