...
"""TURTO 2.2.19 incremental deep-HIT installer; actions.sqlite3 is never modified."""
...
PAYLOADS = {
    "app_runtime.pyw": (
        "96365d73cf7513badaac2def8ba9b0c6cea10238",
        "updates/2.2.19/app_runtime.pyw",
        "319e2f2bc5fafb2dbb7de5888c6af20beeb28d2e0dba89b544fb1a9f61f56ce6",
    ),
    "hit_workspace.py": (
    ...
    "cleanup_stage7.py": (
        "8da57a1e8ac7fc6e7c966e60b56c1149fb013d92",
        "updates/2.2.19/cleanup_stage7.py",
        "6f94e2292fa3dabeff14d62b31738b005bc6e341abca1f5e67ea69db7553f3bb",
    ),
}

# A healthy 2.2.18 installation can use the fast path. The helpers listed here
# were previously transitive dependencies; 2.2.19 makes them explicit so a
# partially damaged Program is repaired instead of being accepted as complete.
PREVIOUS_REQUIRED = (
    "app_runtime.pyw",
...
def _ensure_baseline(root: Path, program: Path, temp: Path) -> None:
    if (
        _revision(program, PREVIOUS_REVISION, "2.2.18")
        and _complete(program, PREVIOUS_REQUIRED)
    ):
        return

    # Fail-safe recovery: rebuild the last full verified baseline. 2.2.16
    # transitively reconstructs the complete Program payload from the verified
    # historical chain, including the helper modules now made explicit.
    base = temp / "base_installer.py"
    base.write_bytes(
        _download(BASE_COMMIT, BASE_PATH, BASE_SHA256, "TURTO-2.2.19-base")
    )
...
def _apply_incremental_overlay(program: Path, temp: Path) -> None:
    ...
    for local, (commit, remote, expected) in PAYLOADS.items():
        ...
        source.write_bytes(
            _download(commit, remote, expected, "TURTO-2.2.19-runtime")
        )
        staged[local] = source
...
def install_runtime(root: Path | str) -> None:
    ...
    if (
        _revision(program, PROGRAM_REVISION, "2.2.19")
    ...
    with tempfile.TemporaryDirectory(prefix="turto_2219_") as temp_name:
        temp = Path(temp_name)
        _ensure_baseline(root, program, temp)
        _apply_incremental_overlay(program, temp)

        if not _complete(program, CURRENT_REQUIRED):
            raise RuntimeError("Runtime 2.2.19 není po inkrementální aktualizaci kompletní.")

        current_marker = program / PROGRAM_REVISION
        current_marker.write_text("2.2.19", encoding="utf-8")
