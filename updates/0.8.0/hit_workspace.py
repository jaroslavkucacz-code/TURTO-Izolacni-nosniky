...
class HitWorkspaceMixin:
    ...
    def _build_hit_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(top, text=f"HIT-HP/SP • MVX / ZVX / ZDX / DD / DVL / DDL • modul {HIT_MODULE_VERSION}", style="MutedCard.TLabel").grid(row=1, column=0, sticky="w", pady=(3, 0))
    ...
    def copy_hit_results(self) -> None:
        ...
        for row in self.hit_rows:
            ...
            lines.append("\t".join([row.name.get(), row.series.get(), row.connection_type.get(), row.height.get(), row.cover.get(), row.concrete.get(), row.med.get(), row.ved.get(), candidate.designation, fmt(candidate.utilization * 100.0) + " %", str(candidate.page), fmt(candidate.m1), fmt(candidate.v1), fmt(candidate.m2), fmt(candidate.v2)]))
            count += 1
        if not count:
