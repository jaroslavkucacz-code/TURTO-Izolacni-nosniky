...
"""TURTO 3.0.16: restored original catalogues; existing customer files are read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_16.ok"

PAYLOADS = {'app_runtime.pyw': ('f74bd1515718dc167ecc0c4fd881ced8527f49c2',
                     'updates/3.0.16/app_runtime.pyw',
                     '0ac8c51f238190caeeabe2cea718c16fde855b4109c0b78404da861d1cb208fb'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.16-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_316_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_316_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.16 neprošel závěrečnou kontrolou.")
    except Exception:
