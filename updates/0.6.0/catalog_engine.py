...
class CatalogDatabase:
        ...
        for fscore, fam in family_rank[:8]:
            ...
            for product in chosen_products:
                ...
                for concrete in concretes:
                    ...
                    # Match any typed shear token exactly/compactly; otherwise provide a few distinct options.
    ...
    def suggestion_hint(self, text: str, preferred_catalog_id: str | None = None, limit: int = 4) -> str:
    ...
    def resolve_designation(self, text: str, preferred_catalog_id: str | None = None) -> QueryResult:
        ...
        if len(exact)>1:
            raise SelectionError("Označení odpovídá více katalogovým záznamům. Upřesněte vydání nebo variantu." + self.suggestion_hint(text, preferred_catalog_id))
        ...
        if parsed.get("type") and parsed.get("moment_class"):
            ...
            if len(recs)>1:
                raise SelectionError("Označení je nejednoznačné. Doplňte chybějící třídu, krytí/variantu nebo rozměr." + self.suggestion_hint(text, preferred_catalog_id))
        raise SelectionError("Označení se nepodařilo jednoznačně přiřadit. Vyberte některou z nabízených možností nebo použijte detailní výběr." + self.suggestion_hint(text, preferred_catalog_id))
