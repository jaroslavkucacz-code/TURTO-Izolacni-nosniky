...
"""TURTO 2.1.5 multi-domain workspace."""
...
import shear_dowels_schedule_guard  # noqa: F401 - unit guard before schedule/UI imports
    ...
    for widget in _walk(owner):
        try:
            ...
            if text in {
                ...
                "TURTO 2.1.4 | Technické prvky",
            }:
                widget.configure(text="TURTO 2.1.5 | Technické prvky")
...
def install(app_base: Any) -> None:
    ...
    if getattr(cls, "_turto_platform_workspace_215_installed", False):
        return
    ...
    def body(self) -> None:
        ...
        try:
            self.base_window_title = "TURTO 2.1.5 – Technické prvky"
            self._update_project_title()
        except Exception:
            pass

    cls._build_body = body
    cls._turto_platform_workspace_215_installed = True
