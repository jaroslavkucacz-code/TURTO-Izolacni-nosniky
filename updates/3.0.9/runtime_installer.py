...
"""TURTO 3.0.9: manual HIT selection preservation; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_9.ok"

PAYLOADS = {'app_runtime.pyw': ('048cc51fd5cc84e1432c49859b82003bfa550c54',
                     'updates/3.0.9/app_runtime.pyw',
                     'aeab7315b057e25cd07aefffe6987baf9441c59444855875ca2cd8deb0cc158b'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.9-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_308_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_308_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.9 neprošel závěrečnou kontrolou.")
    except Exception:
...
def selftest() -> None:
    assert VERSION == "3.0.9" and RUNTIME_LAYOUT == "40"
