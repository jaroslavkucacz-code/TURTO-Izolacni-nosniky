...
"""TURTO 3.0.11: smooth HIT scrolling; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_11.ok"

PAYLOADS = {'app_runtime.pyw': ('75f9662999ff85bfd37ef5310821759dc643d3a6',
                     'updates/3.0.11/app_runtime.pyw',
                     '9f9c0a9b6c0ea3ef5851ecf59cb57b635e2530028d420917050a0d2db4b1a91c'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.11-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_311_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_311_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.11 neprošel závěrečnou kontrolou.")
    except Exception:
