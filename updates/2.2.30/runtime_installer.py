...
"""TURTO 2.2.30 – Peikko EBEA/TEBEA overlay; actions.sqlite3 is never modified."""
...
BASE_COMMIT = 'e345a1a8d9e2a509dde698892c9cfbb0c4755a59'
BASE_PATH = 'updates/2.2.29/runtime_installer.py'
BASE_SHA256 = '53221776b8875d68d48e4cce044850ba20e49dc2b851294f312fe6b555136375'
RUNTIME_LAYOUT = "21"
PROGRAM_REVISION = ".turto_runtime_2_2_30.ok"

PAYLOADS = {'app_runtime.pyw': ('5ebd1d46e0c2073f463d8e40f7f423bb051883b6', 'updates/2.2.30/app_runtime.pyw', 'ecef65e098f5e6cdc0c8095bed1d3c09787aa8e1300a081ed67d105fc5d6a6c9'), 'peikko_thermal_breaks.py': ('5ebd1d46e0c2073f463d8e40f7f423bb051883b6', 'updates/2.2.30/peikko_thermal_breaks.py', '08fd227a62d4398cdaa50c6f05bac1226c7e6d19801bba8f1c371f901b0c0d1b'), 'peikko_catalog.py': ('5ebd1d46e0c2073f463d8e40f7f423bb051883b6', 'updates/2.2.30/peikko_catalog.py', 'bf329aae1da7cee891fb2bc3173e0356c3045a17d0b9ec5446c567cabf8d9e16'), 'peikko_workspace.py': ('ebeedb732b977a95185f05f076eeabef2941ab6b', 'updates/2.2.30/peikko_workspace.py', 'f727fbbfb99fa2ec7d645c0915369105f835914f4c6b5fad12ca6591028d5245'), 'peikko_technical.py': ('5ebd1d46e0c2073f463d8e40f7f423bb051883b6', 'updates/2.2.30/peikko_technical.py', '0ea50aa08c4a68ef00c2a1362ec3bf3aa2fac59b0646bde7c1484e25f174a25f'), 'peikko_documents.py': ('5ebd1d46e0c2073f463d8e40f7f423bb051883b6', 'updates/2.2.30/peikko_documents.py', 'cedd9c09815c8d03e0f454237c1d9a634fed5fb47fd15d7b6b785370c5a20d42'), 'peikko_technical_data.json.gz': ('5ebd1d46e0c2073f463d8e40f7f423bb051883b6', 'updates/2.2.30/peikko_technical_data.json.gz', '1cbf686e5e3153f28cc09648b1abdf7cf3a93e0fe1b14bc1f5b5889fcef51690')}
...
def _revision_ok(program: Path) -> bool:
    try:
        ...
        return (
            marker.is_file()
            and marker.read_text(encoding="utf-8").strip() == "2.2.30"
...
def _install_base(root: Path, temp: Path) -> None:
    ...
    if _complete(program, PREVIOUS_REQUIRED) and _baseline_valid(program):
        return
    installer = temp / "runtime_229.py"
    installer.write_bytes(_download(BASE_COMMIT, BASE_PATH, BASE_SHA256, "TURTO-2.2.30-base"))
    namespace = runpy.run_path(str(installer))
    install = namespace.get("install_runtime")
    if not callable(install):
        raise RuntimeError("Installer 2.2.29 neobsahuje install_runtime().")
    install(root)
    program = root / "Program"
    if not _complete(program, PREVIOUS_REQUIRED):
        raise RuntimeError("Ověřený runtime 2.2.29 se nepodařilo obnovit kompletně.")
...
def _install_payloads(program: Path, temp: Path) -> None:
    ...
    for local, (commit, remote, expected) in PAYLOADS.items():
        ...
        source.write_bytes(_download(commit, remote, expected, "TURTO-2.2.30-runtime"))
...
def install_runtime(root: Path | str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="turto_2230_") as temp_name:
        temp = Path(temp_name)
        ...
        if not _complete(program, CURRENT_REQUIRED):
            raise RuntimeError("Runtime 2.2.30 není po aktualizaci kompletní.")
        if not _payloads_match(program):
            raise RuntimeError("Kontrola runtime 2.2.30 po instalaci selhala.")

        marker = program / PROGRAM_REVISION
        marker.write_text("2.2.30", encoding="utf-8")
