...
"""TURTO 3.0.14: unified Schöck decoder and startup progress; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_14.ok"

PAYLOADS = {'app_runtime.pyw': ('2e4b2450424243a0ce8682a29c62da7f95ee739b',
                     'updates/3.0.14/app_runtime.pyw',
                     '62830326d2d2eccf943c64c7701966221001747b81db97350ba03fbfc9d8dec2'),
 ...
 'catalog_engine.py': ('38af36ed6c43d38dac9f1a4d32167af0a43a2710',
                       'updates/3.0.13/catalog_engine.py',
                       'ca18f1793c32809d834ef16af7c3c8f7ec134b9495f0341ef636bccfb847374d'),
 ...
 'decoder_314.py': ('2e4b2450424243a0ce8682a29c62da7f95ee739b',
                    'updates/3.0.14/decoder_314.py',
                    '5c705e3bc7698cbf07716d968c2af617b5ea5392563dd1b4cda6ecc148562ef0')}
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.14-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_314_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_314_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.14 neprošel závěrečnou kontrolou.")
    except Exception:
...
def selftest() -> None:
    assert VERSION == "3.0.14" and RUNTIME_LAYOUT == "45"
