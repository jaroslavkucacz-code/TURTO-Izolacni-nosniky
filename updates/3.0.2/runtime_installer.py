...
"""TURTO 3.0.2: verified incremental ISO/logo delivery; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_2.ok"

PAYLOADS = {'app_runtime.pyw': ('e6a90331abdac9ed952aae6936c7adab66950b08', 'updates/3.0.2/app_runtime.pyw', '62015a586cbfe8166cf833e509a1864681ce0d4683bd317389fc6566ab94023b'),
 ...
 'substitution_workspace.py': ('b0cf7a80dcd741c1da8815b99eebad010d827fe2', 'updates/1.1.17/substitution_workspace.py', 'dcfe3fa70fb4748c29216c35bce610681fe787331908a219a2f7ba9666082301'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.2-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_302_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_302_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.2 neprošel závěrečnou kontrolou.")
    except Exception:
...
def selftest() -> None:
    assert VERSION == "3.0.2" and RUNTIME_LAYOUT == "33"
