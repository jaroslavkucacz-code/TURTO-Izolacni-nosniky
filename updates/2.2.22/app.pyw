...
"""TURTO 2.2.22 bootstrap – safe release-note ownership across online updates."""
...
INSTALLER_COMMIT = "0c7616459c0d26ee49fd67b031f0a29d128fc60a"
INSTALLER_SHA256 = "40f96de2f20e3b2c1540a99d17df0da39dcdc5f10cb541f24e3d06e9e3151bfa"
RUNTIME_LAYOUT = "15"

ROOT = Path(__file__).resolve().parent
PROGRAM = ROOT / "Program"
MARKER = ROOT / ".turto_runtime_current.ok"
PROGRAM_REVISION = PROGRAM / ".turto_runtime_2_2_22.ok"
CLEANUP_REVISION = ROOT / ".turto_cleanup_2_2_22.ok"
LOG_DIR = ROOT / "Logy"
...
def _download_installer() -> Path:
    url = f"https://raw.githubusercontent.com/{REPOSITORY}/{INSTALLER_COMMIT}/updates/2.2.22/runtime_installer.py"
    last = None
    ...
    for attempt in range(4):
        try:
            request = urllib.request.Request(
                ...
                headers={"User-Agent": "TURTO-2.2.22-Bootstrap", "Cache-Control": "no-cache, no-store", "Pragma": "no-cache"},
            )
...
def _install_runtime() -> None:
    ...
    try:
        ...
        if not PROGRAM_REVISION.is_file():
            raise RuntimeError("Runtime revize 2.2.22 nebyla vytvořena.")
...
def _post_run_cleanup() -> None:
    # Critical race guard: an older still-running process must never consume
    # RELEASE_NOTES.txt that the updater has already replaced for a newer version.
