...
"""TURTO 3.0.17: restored original catalogues; existing customer files are read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_17.ok"

PAYLOADS = {'app_runtime.pyw': ('02534506bd62606728f4494481ce0778987745ed',
                     'updates/3.0.17/app_runtime.pyw',
                     'fcaceca7891d423379557b25ee9791adc818ef301c2f470b55a88bbaa73e343b'),
 ...
 'workspace_controls_317.py': ('8ebac5e14fb7ad0d632033092e5ad4ed1656f89f', 'updates/3.0.17/workspace_controls_317.py', '205cfe7631a2c4b98b0481861e783ae646db1d8f8347c8530b9fda9bcb18bc7d'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.17-runtime", "Cache-Control": "no-cache, no-store"})
...
def install_runtime(root: Path | str) -> Path:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.17 neprošel závěrečnou kontrolou.")
    except Exception:
...
def selftest() -> None:
    assert VERSION == "3.0.17" and RUNTIME_LAYOUT == "48"
