...
"""TURTO 3.0.15: restored original catalogues; existing customer files are read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_15.ok"

PAYLOADS = {'app_runtime.pyw': ('d321d428680ddbad888ed8eaad385d6ee642ffff',
                     'updates/3.0.15/app_runtime.pyw',
                     'b8c482977d0e5ebd0084a6fdbc411d6d1e521ae2629499d5f87c81c1df200724'),
 ...
 'project_model.py': ('d321d428680ddbad888ed8eaad385d6ee642ffff',
                      'updates/3.0.15/project_model.py',
                      '79618c83a9e0c88e35824db75abf6f04bcbf2ffef82707759e215a20b7ac2b4a'),
 ...
 'decoder_315.py': ('d321d428680ddbad888ed8eaad385d6ee642ffff',
                    'updates/3.0.15/decoder_315.py',
                    'b04ec193122af1a5766464a553558ac54271dcd137a216a709c5e889e9e44c92'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.15-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_315_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_315_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.15 neprošel závěrečnou kontrolou.")
    except Exception:
...
def selftest() -> None:
    assert VERSION == "3.0.15" and RUNTIME_LAYOUT == "46"
