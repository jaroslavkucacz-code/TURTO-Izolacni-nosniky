...
class CatalogDatabase:
    ...
    def __init__(self, catalog_directory: Path | str):
        ...
        self._suggestion_token_index: dict[str, array] = {}
        self._suggestion_vocabulary: set[str] = set()
        self._selector_values: dict[str, tuple[str, ...]] = {}
        self._suggestion_index_ready = False
        self._load()

    def _load(self) -> None:
        # Full original catalogues were omitted when the portable EXE was
        # assembled only from incremental updates. Keep verified release copies
        # beside the engine; existing customer catalogue IDs take precedence.
        bundled = (
            ("isopro_2018_en.json.gz.b64", "isopro-en-2018-user-source"),
            ("schoeck_cz_2024_1_2024_09.json.gz.b64", "schoeck-cz-2024.1-2024-09"),
        )
        ...
        for path in files:
            try:
                ...
                self._share_catalog_strings(package, strings)
        ...
        for name, catalog_id in bundled:
            if catalog_id in self.catalogs:
                continue
            path = Path(__file__).with_name(name)
            try:
                package = json.loads(gzip.decompress(base64.b64decode(path.read_bytes())).decode("utf-8"))
                if package["catalog"]["id"] != catalog_id:
                    raise CatalogError(f"Neočekávané ID základního katalogu: {name}")
                self._share_catalog_strings(package, strings)
                self._load_package(path, package)
            except Exception as exc:
                self.load_errors.append(f"Základní katalog {name}: {exc}")
        ...
        self._build_selector_value_cache()

    @staticmethod
    def _share_catalog_strings(value: Any, strings: dict[str, str]) -> Any:
        """Reuse equal immutable text; keep every mutable record/result separate.

        The temporary pool belongs only to loading and is released afterwards.
        Catalogues, units, numbers and source references remain identical.
        """
        if isinstance(value, str):
            return strings.setdefault(value, value)
        if isinstance(value, dict):
            for key, child in value.items():
                value[key] = CatalogDatabase._share_catalog_strings(child, strings)
        elif isinstance(value, list):
            for index, child in enumerate(value):
                value[index] = CatalogDatabase._share_catalog_strings(child, strings)
        return value
    ...
    def _build_suggestion_index(self) -> None:
        """Invalidate after loading or extending catalogues, without eager work."""
        self._suggestion_entries.clear()
        self._suggestion_token_index.clear()
        self._suggestion_vocabulary.clear()
        self._suggestion_index_ready = False

    def _ensure_suggestion_index(self) -> None:
        if self._suggestion_index_ready:
            return
        # Build locally, so an interrupted build cannot leave partial entries.
        entries: list[dict[str, Any]] = []
        token_index: dict[str, array] = {}
            ...
            for record in fam.get("records", []):
                ...
                for token in tokens:
                    if token not in token_index:
                        token_index[token] = array("I")
                    token_index[token].append(idx)
                    vocabulary.add(token)
        self._suggestion_entries = entries
        self._suggestion_token_index = token_index
        self._suggestion_vocabulary = vocabulary
        self._suggestion_index_ready = True

    def _build_selector_value_cache(self) -> None:
        """Předpočítá unikátní hodnoty selektorů jednou při načtení databáze.

        Hromadný import dříve při každém řádku znovu procházel všechny
        nematicové katalogové záznamy, jen aby zjistil, zda text obsahuje
        výrobce, řadu, typ, třídu nebo krytí. Nyní se porovnává pouze s malou
        množinou unikátních hodnot.
        """
        families = [family for family in self.families if family.get("backend") != "matrix"]
        records = [record for family in families for record in family.get("records", [])]

        def unique(values: Iterable[Any]) -> tuple[str, ...]:
            clean = {str(value).strip() for value in values if str(value or "").strip() not in {"", "—"}}
            return tuple(sorted(clean, key=natural_key))

        self._selector_values = {
            "manufacturer": unique(family.get("manufacturer") for family in families),
            "model": unique(family.get("model") for family in families),
            "type": unique(family.get("type") for family in families),
            "generation": unique(family.get("generation") for family in families),
            "moment_class": unique(record.get("moment_class") for record in records),
            "shear_class": unique(record.get("shear_class") for record in records),
            "cover": unique(record.get("cover") for record in records),
        }
    ...
    def _explicit_selector_constraints(self, text: str) -> dict[str, set[str]]:
        ...
        for key in ("manufacturer", "model", "type", "generation", "moment_class", "shear_class", "cover"):
            found = find_values(self._selector_values.get(key, ()))
            if found:
