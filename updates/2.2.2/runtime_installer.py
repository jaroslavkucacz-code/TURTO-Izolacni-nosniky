...
"""TURTO 2.2.2 – aktuální instalační cesta runtime.
...
def _install_verified_base(root: Path) -> None:
    ...
    try:
        ...
        installer.write_bytes(_download(url, "TURTO-2.2.2-base"))
...
def install_runtime(root: Path | str) -> None:
    ...
    window = tk.Tk()
    window.title("TURTO 2.2.2 – obnova runtime")
    ...
    try:
        for index, (local, (commit, remote, expected)) in enumerate(PAYLOADS.items(), 1):
            ...
            source.write_bytes(
                _download_checked(commit, remote, expected, "TURTO-2.2.2-runtime")
            )
