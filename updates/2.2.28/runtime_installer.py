...
"""TURTO 2.2.28 – Peikko EBEA/TEBEA overlay; actions.sqlite3 is never modified."""
...
BASE_SHA256 = "cdfa34ce9b5ea3423710fe7d862522da7f54a733c6f3a04621ef48d9b8851237"
RUNTIME_LAYOUT = "21"
...
PAYLOADS = {
    ...
    "app_runtime.pyw": (
        "f92431564560dbe715d915c3ccd3412cc5fee8c2",
        "updates/2.2.28/app_runtime.pyw",
        "fc353328509955a31fcbdfd58baff04a469d31948a9f1a5d05210a33d5716af1",
    ),
    "peikko_thermal_breaks.py": (
        "94fe366840c495e3dec197a3d109cd86ab316671",
        "updates/2.2.28/peikko_thermal_breaks.py",
        "2e086653870c5bd6e626e09f78d256b7f7aa213a3a23fcb3c590a0a0d196f2cb",
    ),
    "peikko_workspace.py": (
        "83e575437bda44f2069c56dbeb2f6f8136ba7942",
        "updates/2.2.28/peikko_workspace.py",
        "c9f8c72aba6c7646ef56c71024185337c73244ba9e24e1e7ed39abaa38757197",
    ),
}
...
def _install_base(root: Path, temp: Path) -> None:
    installer = temp / "runtime_227.py"
    installer.write_bytes(_download(BASE_COMMIT, BASE_PATH, BASE_SHA256, "TURTO-2.2.28-base"))
...
def _install_payloads(program: Path, temp: Path) -> None:
    ...
    for local, (commit, remote, expected) in PAYLOADS.items():
        ...
        source.write_bytes(_download(commit, remote, expected, "TURTO-2.2.28-runtime"))
...
def install_runtime(root: Path | str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="turto_2228_") as temp_name:
        ...
        if not _payloads_match(program):
            raise RuntimeError("Kontrola runtime 2.2.28 po instalaci selhala.")
