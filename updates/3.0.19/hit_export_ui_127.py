...
def collect_all_hit_rows(owner: Any) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(getattr(owner, "wt_rows", []), 1):
        typ = row.connection_type.get() if hasattr(row, "connection_type") else "WT"
        ...
        item = {
            "group": "Nosníky ST" if typ == "ST" else "Stěny WT",
            ...
            "detail": status_text,
            "notes": (
                [f"{typ}: při současném působení více účinků je nutná kontrola jejich kombinace."]
