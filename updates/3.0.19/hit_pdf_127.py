...
def _input_summary(row: dict[str, Any], *, with_actions: bool = True) -> str:
    if str(row.get("group", "")).strip() == "Stěny WT" or str(row.get("connection_type", "")).upper() in {"ST", "WT"}:
        parts = [
            f"{_prev._text(row.get('series'))} {_prev._text(row.get('connection_type'))}",
            f"h skutečné {_prev._text(row.get('height_mm'))} mm",
...
def _catalog_table(self, row: dict[str, Any], width: float):
    ...
    if typ in {"ST", "WT"}:
        ...
        if typ == "ST":
            entries = [item for item in entries if item[0] != "VRd,h"]
...
def _card_flows(self, row: dict[str, Any], refs: list[int]):
    ...
    flows.append(self.p(
        ...
        f"h skutečné: {_prev._num(candidate.get('height'), 0)} mm  •  "
        f"{_prev._text(candidate.get('concrete'))}  •  zdroj HIT20.2 p.{_prev._text(candidate.get('page'))}"
    ))
    flows.append(self.p(str(candidate.get("height_note", "")), size=10.5))
    ...
    if status == "KONTROLA":
        flows.append(self.p(
            f"Současné působení více složek {typ}: dílčí tabulované únosnosti vyhovují, ale společná interakce není bez ověřeného katalogového pravidla automaticky prohlášena za vyhovující.",
            size=10.5,
            color=_base.WARNING,
        ))
    if refs:
