...
from hit_wt import WtCandidate, design_connector
from xlsx_export import write_xlsx

WT_HEADERS = (
    ("name","Pozice","w"),("quantity","Ks","center"),("series","Řada","center"),("connection_type","Typ","center"),
    ("wall_height","Výška h [mm]","center"),("table_height","h tab. [mm]","center"),("width","Šířka B [mm]","center"),("concrete","Beton","center"),
    ("med_neg","MEd− [kNm/prvek]","center"),("ved_vertical","VEd,v+ [kN/prvek]","center"),("ved_horizontal","VEd,h± WT [kN/prvek]","center"),
    ("product","Návrh HIT-ST / WT","w"),("util","Využití","center"),("capacity","MRd / VRd,v / VRd,h","center"),
...
class WtRow:
    def __init__(self, owner:Any, parent:ttk.Frame, row_no:int, defaults:dict[str,Any]|None=None)->None:
        ...
        self.connection_type=tk.StringVar(master=owner,value=str(d.get("connection_type","WT")))
        ...
        self.table_height=tk.StringVar(master=owner,value="—")
    ...
    def _build(self):
        self._entry(self.name,9,"left"); self._entry(self.quantity,5); self._combo(self.series,("HP","SP"),5); self.type_combo=self._combo(self.connection_type,("ST","WT"),5)
        self._entry(self.wall_height,9); self.widgets.append(ttk.Label(self.parent,textvariable=self.table_height,width=9,anchor="center",style="Card.TLabel")); self._entry(self.width,8); self._combo(self.concrete,("C20/25","C25/30","C30/37"),9)
    ...
    def recalculate(self,preserve:str|None=None):
        ...
        rows,error=design_connector(connection_type=self.connection_type.get(),series=self.series.get(),wall_height_mm=h,width_mm=b,concrete=self.concrete.get(),med_neg=m,ved_vertical=vv,ved_horizontal=vh)
    ...
    def _show(self,row:WtCandidate):
        ...
        vh = "—" if row.vrd_horizontal is None else f"{row.vrd_horizontal:.1f}"
        self.capacity.set(f"{row.mrd:.1f} / {row.vrd_vertical:.1f} / {vh}".replace(".",",")); self.joint.set(f"{row.joint_spacing_m:.1f} m".replace(".",",")); self.source.set(f"HIT20.2 p.{row.page}"); self.table_height.set(str(row.table_height_mm))
        self.status.set(row.mode+(" • ručně zvoleno" if self.manual_product else "")+(" • kontroly M/V samostatně" if row.connection_type=="ST" else " • kontroly M/Vv/Vh samostatně")); self.status_label.configure(style="Good.TLabel")
...
def install(base:Any)->None:
    ...
    def build_wt(self,parent):
        ...
        ttk.Label(head,text="Nosníky ST / stěny WT",style="ProjectTitle.TLabel").grid(row=0,column=0,sticky="w")
        ttk.Label(head,text="Účinky na jeden prvek: MEd− a VEd,v+. WT navíc VEd,h±. Zadejte skutečnou výšku h a šířku B; únosnost se bere z nejbližší nižší tabulkové výšky.",style="MutedCard.TLabel").grid(row=1,column=0,sticky="w",pady=(3,0))
        bar=ttk.Frame(parent,style="App.TFrame"); bar.grid(row=1,column=0,sticky="ew",pady=(10,8))
        ttk.Button(bar,text="+ Přidat ST",style="Accent.TButton",command=lambda:self.add_wt_row({"connection_type":"ST","name":f"ST{len(self.wt_rows)+1:03d}","wall_height":"400","width":"220"})).grid(row=0,column=0); ttk.Button(bar,text="+ Přidat WT",style="Accent.TButton",command=self.add_wt_row).grid(row=0,column=1); ttk.Button(bar,text="Přepočítat vše",command=self.recalculate_wt_all).grid(row=0,column=2,padx=(7,0)); ttk.Button(bar,text="Vymazat vše",command=self.clear_wt_rows).grid(row=0,column=3,padx=(7,0)); ttk.Button(bar,text="Kopírovat tabulku",command=self.copy_wt_table).grid(row=0,column=4,padx=(14,0)); ttk.Button(bar,text="Export Excel",command=self.export_wt_excel).grid(row=0,column=5,padx=(7,0)); bar.columnconfigure(20,weight=1); ttk.Label(bar,textvariable=self.wt_status_var,style="Muted.TLabel").grid(row=0,column=20,sticky="e")
        ...
        ttk.Label(parent,text="ST: h = 400–1000 mm, B = 220–340 mm. WT1–4: h = 1250–3500 mm; WT5–7: h = 1000–3500 mm, B = 150–250 mm. Mezilehlé h: únosnost nižší tabulkové výšky (ST po 100 mm, WT po 250 mm), bez interpolace a bez extrapolace. Více účinků vyžaduje kontrolu kombinace.",style="Muted.TLabel",wraplength=1500,justify="left").grid(row=3,column=0,sticky="ew",pady=(8,0))
    ...
    def export_data(self):
        headers=["Pozice","Ks","Typ","Řada","h skutečné [mm]","h tab. pro únosnost [mm]","B [mm]","Beton","MEd− [kNm/prvek]","VEd,v+ [kN/prvek]","VEd,h± WT [kN/prvek]","Návrh HIT","Využití [%]","MRd [kNm/prvek]","VRd,v [kN/prvek]","VRd,h [kN/prvek]","s_joint max. [m]","Zdroj / výška","Kontrola"]
        rows=[]
        for r in self.wt_rows:
            c=r.selected_candidate
            rows.append([r.name.get(),_qty(r.quantity.get()),r.connection_type.get(),r.series.get(),r.wall_height.get(),c.table_height_mm if c else "",r.width.get(),r.concrete.get(),r.med_neg.get(),r.ved_vertical.get(),r.ved_horizontal.get(),c.designation if c else "—",round(c.utilization*100,1) if c else "",c.mrd if c else "",c.vrd_vertical if c else "",c.vrd_horizontal if c and c.vrd_horizontal is not None else "—",c.joint_spacing_m if c else "",f"HIT20.2 p.{c.page}; {c.height_note}" if c else "",r.status.get()])
        return headers,rows
    def copy(self):
        headers,rows=export_data(self)
        self.clipboard_clear();self.clipboard_append("\n".join("\t".join(map(str,row)) for row in [headers,*rows]))
        self.set_status(f"ST / WT: zkopírováno {len(rows)} řádků.")
    def export(self):
        if not self.wt_rows:messagebox.showinfo("ST / WT","Není co exportovat.",parent=self);return
        headers,rows=export_data(self)
        name=getattr(getattr(self,"project",None),"name","AKCE") or "AKCE"
        safe=re.sub(r'[<>:"/\\|?*]+',"_",str(name)).strip(" ._") or "AKCE"
        path=filedialog.asksaveasfilename(parent=self,title="Export HIT-ST / WT do Excelu",initialfile=safe+"_HIT_ST_WT.xlsx",defaultextension=".xlsx",filetypes=[("Excel sešit","*.xlsx")])
        if not path:return
        try:saved=write_xlsx(path,headers=headers,rows=rows,sheet_name="HIT-ST-WT",creator="TURTO – Vytvořil Ing. Jaroslav Kučera")
        except Exception as exc:messagebox.showerror("Export HIT-ST / WT",str(exc),parent=self);return
        try:
            if os.name=="nt":os.startfile(str(Path(saved).resolve()))
        except Exception:pass
        self.set_status(f"ST / WT exportován do {Path(saved).name}.")
    base.HitWorkspaceMixin.wt_export_data=export_data
