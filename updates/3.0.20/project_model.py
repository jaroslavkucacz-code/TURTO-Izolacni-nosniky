...
from catalog_engine import CatalogDatabase, QueryResult, SelectionError
...
def query_from_selection(database: CatalogDatabase, selection: dict[str, Any]) -> QueryResult:
    fields = {key: str(selection[key]) for key in (
        "catalog_id", "manufacturer", "model", "type_name", "generation",
        "moment_class", "concrete_min", "shear_class", "cover", "height_mm")}
    try:
        return database.query(**fields)
    except SelectionError:
        # Older saved Schöck shear rows stored H240/H250 instead of the
        # catalogue's >= minimum-height key. Resolve only one matching row
        # from the SAME edition and with ALL other selectors unchanged.
        # Never rewrite the saved selection, snapshot or accepted HIT choice.
        if (fields["catalog_id"] != "schoeck-cz-2024.1-2024-09"
                or fields["manufacturer"] != "Schöck"
                or fields["model"] not in {"T", "XT"}
                or fields["type_name"] not in {"QL", "QP", "QP-Z"}
                or not fields["height_mm"].isdigit()):
            raise
        from iso_bulk_301 import height_matches
        family = database.get_family(*(fields[k] for k in (
            "catalog_id", "manufacturer", "model", "type_name", "generation")))
        wanted = int(fields["height_mm"])
        matches = [r for r in family.get("records", [])
                   if all(str(r.get(k)) == fields[k] for k in (
                       "moment_class", "concrete_min", "shear_class", "cover"))
                   and height_matches(r.get("height_mm", ""), wanted)]
        if len(matches) != 1:
            raise
        record = dict(matches[0], input_height_mm=wanted)
        return database.result_for_record(family, record)


def create_project_row(result: QueryResult, *, position: str, quantity: int=1, note: str="", source_text: str="", row_id: str|None=None, element_length_mm: int|None=None) -> dict[str,Any]:
    row = {"id":row_id or uuid.uuid4().hex,"position":str(position).strip(),"quantity":safe_quantity(quantity),"note":str(note).strip(),
    ...
    from decoder_length_320 import note_length, set_length
    length = element_length_mm if element_length_mm is not None else note_length(note)
