...
from decoder_length_320 import metres, effective_length, capacity_length_issue
...
class DecoderDetailDialog(tk.Toplevel):
    def __init__(self, owner: Any, row: dict[str, Any]) -> None:
        ...
        items = (
            ...
            ("Délka prvku", (metres(effective_length(row)) + " m") if effective_length(row) else "neuvedena"),
        ...
        if issue: lines.append("• OVĚŘIT DÉLKU: " + issue)
        ...
        def edit_length():
            # The detail wrapper may pass an enriched COPY; edit the actual project row.
            self.destroy()
            if owner.project_tree.exists(str(row.get("id", ""))):
                owner.project_tree.selection_set(str(row["id"]))
                owner.edit_selected_project_metadata()
        ttk.Button(buttons, text="Upravit pozici / ks / délku…", command=edit_length).grid(row=0, column=0, sticky="w")
