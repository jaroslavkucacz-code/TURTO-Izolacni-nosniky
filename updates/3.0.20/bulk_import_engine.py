...
from decoder_length_320 import header_length, is_length_header, parse_length, note_length
...
class BulkAnalysisProgress:
    """Průběžná informace pro neblokující hromadnou analýzu."""
...
def _classification_snapshot(item: BulkImportItem) -> dict[str, Any]:
    return {
        "status": item.status,
        "message": item.message,
        "result": item.result,
        "candidates": list(item.candidates),
        "varying_keys": tuple(item.varying_keys),
        "archive_source": dict(item.archive_source) if isinstance(item.archive_source, dict) else None,
    }


def _apply_classification_snapshot(item: BulkImportItem, snapshot: dict[str, Any]) -> None:
    item.status = str(snapshot.get("status", "error"))
    item.message = str(snapshot.get("message", ""))
    item.result = snapshot.get("result") if isinstance(snapshot.get("result"), QueryResult) else None
    item.candidates = list(snapshot.get("candidates", []))
    item.varying_keys = tuple(snapshot.get("varying_keys", ()))
    source = snapshot.get("archive_source")
    item.archive_source = dict(source) if isinstance(source, dict) else None
...
def _classify_bulk_item(
    ...
    *,
    ...
    exact_candidates = [
        ...
        if is_exact_designation(item.designation, candidate.result)
    ]
    ...
    archive_source = find_schoeck_archive_source(item.designation)
...
def analyze_bulk_text_progressive(
    database: CatalogDatabase,
    text: str,
    *,
    preferred_concrete: str | None,
    ...
    progress_callback: Callable[[BulkAnalysisProgress], None] | None = None,
    cancel_check: Callable[[], bool] | None = None,
) -> tuple[list[BulkImportItem], int, bool]:
    """Analyzuje výkaz po řádcích, hlásí průběh a respektuje storno.

    Storno se vyhodnotí mezi řádky. Aktuálně běžící katalogový dotaz se dokončí,
    aby databáze zůstala v konzistentním stavu. Opakovaná stejná označení se
    během jednoho běhu vyhodnotí pouze jednou a další výskyty použijí cache.
    """
    ...
    completed = 0
    cache_hits = 0
    classification_cache: dict[tuple[str, str], dict[str, Any]] = {}
    length_column = None
    length_header_error = ""
        ...
        *,
        ...
        progress_callback(
            BulkAnalysisProgress(
                current=current,
                total=total,
                completed=completed if completed_value is None else completed_value,
    ...
    for current, (line_number, raw_line) in enumerate(raw_lines, start=1):
        if cancel_check is not None and cancel_check():
            notify(
                current=current,
                line_number=line_number,
                designation="",
                phase="Analýza zastavena",
            )
            return items, skipped_headers, True

        parts = split_bulk_line(raw_line)
        if looks_like_bulk_header(parts) or is_length_header(parts):
            try:
                length_column = header_length(parts)
                length_header_error = ""
            except ValueError as exc:
                length_header_error = str(exc)
            skipped_headers += 1
            completed = current
            notify(
                current=current,
        ...
        length = None
        try:
            if length_header_error:
                raise ValueError(length_header_error)
            if length_column is not None:
                column, unit = length_column
                length = parse_length(parts.pop(column) if len(parts) > column else "", unit)
            ...
            extra_length = note_length(note)
            if length is not None and extra_length is not None and length != extra_length:
                raise ValueError("Délka ve sloupci se liší od délky v označení / poznámce.")
            length = length if length is not None else extra_length
            if not designation:
            ...
            notify(
                current=current,
                line_number=line_number,
                designation=raw_line.strip(),
                phase="Chyba vstupního řádku",
                completed_value=completed,
            )
            continue
        ...
        notify(
            ...
            line_number=line_number,
            designation=designation,
            phase="Vyhledávám katalog a typ",
        )

        cache_key = (
            normalize_designation_text(designation),
            str(preferred_concrete or ""),
        )
        cached = classification_cache.get(cache_key)
        if cached is not None:
            _apply_classification_snapshot(item, cached)
            cache_hits += 1
            phase = f"{item.status_text} – výsledek z cache"
        else:
            _classify_bulk_item(
                database,
                item,
            ...
            classification_cache[cache_key] = _classification_snapshot(item)
            phase = item.status_text
    ...
    return items, skipped_headers, False
...
def analyze_bulk_text(
    ...
    *,
    ...
    """Zpětně kompatibilní synchronní obálka."""
    items, skipped_headers, _cancelled = analyze_bulk_text_progressive(
        database,
        text,
        preferred_concrete=preferred_concrete,
