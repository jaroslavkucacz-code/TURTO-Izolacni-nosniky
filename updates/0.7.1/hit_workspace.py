...
class HitWorkspaceMixin:
    ...
    def _build_hit_tab(self, parent: ttk.Frame) -> None:
        ...
        headers = ("Označení", "Řada", "h [mm]", "cnom", "Beton", "MEd [kNm/m]", "VEd [kN/m]", "Navržený výrobek ▼", "Využití", "DoP", "Stav / interakce", "")
        for column, text in enumerate(headers):
            ttk.Label(self.hit_rows_frame, text=text, style="Card.TLabel", font=("Calibri", 10, "bold"), anchor="w" if column in (0, 7, 10) else "center").grid(row=0, column=column, sticky="ew", padx=2, pady=(0, 5))
            self.hit_rows_frame.columnconfigure(column, weight=1 if column in (0, 7, 10) else 0)
