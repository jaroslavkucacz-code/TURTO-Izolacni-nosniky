...
"""TURTO 2.0.1 immutable runtime bootstrap."""
...
VERSION = "2.0.1"
PINNED_COMMIT = "0431c75878dd3fee854917fe2fceee01bf06aea4"
REPOSITORY = "jaroslavkucacz-code/TURTO-Izolacni-nosniky"
ROOT = Path(__file__).resolve().parent
MARKER = ROOT / ".turto_runtime_2_0_1.ok"

FILES = {
    # 2.0.1 hotfix layer
    "app_runtime.pyw": "updates/2.0.1/app_runtime.pyw",
    "hit_workspace.py": "updates/2.0.1/hit_workspace.py",

    # 2.0.0 platform layer
    ...
    # Immutable 1.1.27 compatibility / calculation layer
...
def _install_runtime() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="turto_201_", dir=str(ROOT)))
    window = tk.Tk()
    window.title("TURTO 2.0.1 – dokončení opravy")
    ...
    status = tk.Label(
        window,
        text="Opravuji start platformy 2.0…",
        font=("Calibri", 10),
        bg="#F3F6F9",
        fg="#172230",
        justify="left",
...
def _show_failure(exc: Exception) -> None:
    try:
        ...
        messagebox.showerror(
            "Aktualizace TURTO 2.0.1",
            "Dokončení opravy se nezdařilo. Centrální databáze AKCÍ ani katalogy nebyly měněny.\n\n"
            + str(exc)
...
def main() -> int:
    ...
    if not target.exists():
        _show_failure(RuntimeError("Chybí app_runtime.pyw po dokončení opravy."))
        return 3
