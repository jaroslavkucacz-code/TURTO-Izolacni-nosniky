...
"""TURTO 3.0.3: verified Annex 3 parser and supplement delivery; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_3.ok"

PAYLOADS = {'app_runtime.pyw': ('77b12435c6d6fe82c9f16a3b8a270319ffb4f324',
                     'updates/3.0.3/app_runtime.pyw',
                     '77192b491a848f4da003beac8051c9cdf6659781aca1953bbd982040510f84a9'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.3-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_303_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_303_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.3 neprošel závěrečnou kontrolou.")
    except Exception:
