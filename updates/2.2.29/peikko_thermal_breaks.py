...
TURTO 2.2.29 intentionally does not contain Peikko resistance tables.
...
class DecodedPeikko:
    ...
    def parameters(self) -> dict:
        return {key: getattr(self, key) for key in (
            "variant", "material", "reinforcement", "ds_mm", "dt_mm", "sw_mm",
...
def decode_peikko(text: str) -> DecodedPeikko | None:
    ...
    for token, field in (("DS", "ds_mm"), ("DT", "dt_mm"), ("SW", "sw_mm"),
    ...
    for token, key in (("Ds", "ds_mm"), ("Dt", "dt_mm"), ("SW", "sw_mm"),
                       ("L", "length_mm"), ("S11=", "s11_mm"), ("CV", "cover_mm")):
    ...
    for line in lines:
        if is_peikko(line):
            ...
            current = line
                ...
                r"^(?:Ds|Dt|SW|L(?=\d|\s*=)|S11|REI|OQ\b|CV|RS\b|VE[12]\b|C\d+/)", line, re.I):
...
def format_decode(decoded: DecodedPeikko) -> str:
    ...
    labels = {"variant": "Varianta", "material": "Provedení výztuže",
              ...
              "dt_mm": "Dt [mm]", "sw_mm": "Izolant SW [mm]", "length_mm": "Délka L [mm]",
    ...
    rows.extend(("Statické únosnosti: NEJSOU OVĚŘENÉ; prázdná hodnota není nula.",
