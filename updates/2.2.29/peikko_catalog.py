...
"""Syntax-only Peikko adapter for the existing catalog/project contract.

A full designation is the type key so every parameter survives even the legacy
selection normalizer. No resistance (including a zero resistance) is fabricated.
"""
...
BLOCK_REASON = (
    "EBEA/TEBEA: chybí ověřené statické únosnosti konkrétní konfigurace. "
    "Automatickou záměnu ani statické vyhovění nelze potvrdit."
)
CATALOG = {"id": CATALOG_ID, "manufacturer": "Peikko", "edition": "Označení – bez únosností",
           "publication_label": "Datová vrstva označení; nikoli statický katalog",
...
def make_result(text: str, concrete: str | None = None) -> QueryResult:
    ...
    record = {"designation": d.canonical, "aliases": [str(text)],
              ...
              "peikko_parameters": d.parameters, "resistance_verified": False}
