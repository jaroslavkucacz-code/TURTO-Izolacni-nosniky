...
class DecoderFileDialog(ShearScheduleDialog):
    ...
    def _build(self, title):
        ...
        for n, (key, label) in enumerate((("slab", "h [mm]"), ("gap", "Návrhová spára [mm]"), ("concrete", "Beton"))):
...
def add_single(owner):
    ...
    rows = parse_rows(line, decoder=base.decode_dowel,
        defaults={key:values[key].get() for key in ('slab','gap','concrete')},
        existing_names={str(r.get('name','')) for r in owner.shear_decoder_rows},
...
def _compact_decoder(owner, ac):
    ...
    for col,(key,label) in enumerate((('slab','h [mm]'),('gap','Návrhová spára [mm]'),('concrete','Beton'))):
