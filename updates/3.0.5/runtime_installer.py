...
"""TURTO 3.0.5: standard company PDF logo delivery; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_5.ok"

PAYLOADS = {'app_runtime.pyw': ('f3993a50d2a79e51e1d88994e056a6c88d9a7931',
                     'updates/3.0.5/app_runtime.pyw',
                     'ac2a8a4e5af5ca32c02cab9dcfb7a2c7c67edcd24ef0dcfb4821e999da587ffe'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.5-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_305_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_305_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.5 neprošel závěrečnou kontrolou.")
    except Exception:
...
def selftest() -> None:
    assert VERSION == "3.0.5" and RUNTIME_LAYOUT == "36"
