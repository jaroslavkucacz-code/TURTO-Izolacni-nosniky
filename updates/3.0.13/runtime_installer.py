...
"""TURTO 3.0.13: compact catalogue memory; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_13.ok"

PAYLOADS = {'app_runtime.pyw': ('38af36ed6c43d38dac9f1a4d32167af0a43a2710',
                     'updates/3.0.13/app_runtime.pyw',
                     '272e6b5d74eb4ee625cf2e93c1191ec1c98ee6f776ee506c55f48b3bc30858fd'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.13-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_313_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_313_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.13 neprošel závěrečnou kontrolou.")
    except Exception:
