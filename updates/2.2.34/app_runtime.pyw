...
"""TURTO 2.2.34 runtime – Leviat/HALFEN HSD in the shared shear-dowel decoder."""
...
BASE_RUNTIME = PROGRAM / "app_runtime_233.pyw"

if str(PROGRAM) not in sys.path:
    sys.path.insert(0, str(PROGRAM))

if not BASE_RUNTIME.is_file():
    raise RuntimeError(f"Chybí ověřený runtime základ 2.2.33: {BASE_RUNTIME}")

_namespace = runpy.run_path(str(BASE_RUNTIME), run_name="turto_app_runtime_233")
_base = _namespace.get("_base")
_hit_workspace = _namespace.get("_hit_workspace")

if _base is None:
    raise RuntimeError("Runtime 2.2.33 neposkytl app_base.")

import hsd_decoder
hsd_decoder.install(_base)
...
def selftest() -> None:
    assert APP_VERSION == "2.2.34"
    assert BASE_RUNTIME.name == "app_runtime_233.pyw"
    assert _base is not None
    hsd_decoder.selftest()
    assert getattr(_base.ThermalConnectorApp, "_design_groups_restore_233", False)
    assert getattr(_base.ThermalConnectorApp, "_leviat_hsd_decoder_234", False)
