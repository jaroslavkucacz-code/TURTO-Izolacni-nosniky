...
"""TURTO 3.0.7: reviewed T-QP-VV records; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_7.ok"

PAYLOADS = {'app_runtime.pyw': ('4ca42cbf8d075f0e7dfbdf98c0d0c73eebaa4746',
                     'updates/3.0.7/app_runtime.pyw',
                     '639d9d363e0f0a6a0548f1aa3b63d86f21b53ed3a203916e8be10122554d50c1'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.7-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_307_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_307_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.7 neprošel závěrečnou kontrolou.")
    except Exception:
...
def selftest() -> None:
    assert VERSION == "3.0.7" and RUNTIME_LAYOUT == "38"
