...
def enable_high_dpi() -> None:
    ...
    if sys.platform != "win32":
        return
    try:
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(ctypes.c_wchar_p("TURTO.ISO"))
    except Exception:
        pass
    try:
...
class ThermalConnectorApp(HitWorkspaceMixin, ProjectWorkspaceMixin, tk.Tk):
    ...
    def _build_header(self) -> None:
        ...
        ttk.Label(header, text="TURTO ISO | Databáze izolačních nosníků", style="HeaderTitle.TLabel").grid(
    ...
    def _project_concrete_class(self) -> str:
