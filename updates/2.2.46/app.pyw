...
"""TURTO 2.2.46 – active GUI binding for family-aware Schöck Isokorb XT bulk resolver."""
import hashlib
import importlib
...
INSTALLER_COMMIT = "00fce83b4abd9912dbf26c113b798c6670fa9241"
INSTALLER_SHA256 = "9c9a435923c0cb4514dedeaaf06a10fb743c3b4201ed999d36b63ee617b2bad2"
RUNTIME_LAYOUT = "30"

ROOT = Path(__file__).resolve().parent
PROGRAM = ROOT / "Program"
ROOT_MARKER = ROOT / ".turto_runtime_current.ok"
PROGRAM_MARKER = PROGRAM / ".turto_runtime_2_2_46.ok"
STARTUP_LOG = ROOT / "Logy" / "startup.log"

CRITICAL_PROGRAM_SHA256 = {
    "app_runtime.pyw": "f9b7647b35b6d33d1aad7222eda0dd6573cb6ce882f942a71645f90a8ae3a5b9",
    "app_runtime_244.pyw": "815ba2950af3145552c0d40ed2771bd3448c0c00d6feab580c321ee9410b4627",
...
def _download_installer() -> Path:
    url = f"https://raw.githubusercontent.com/{REPOSITORY}/{INSTALLER_COMMIT}/updates/2.2.46/runtime_installer.py"
    folder = Path(tempfile.mkdtemp(prefix="turto_2246_boot_"))
    ...
    for attempt in range(4):
        try:
            request = urllib.request.Request(
                ...
                headers={
                    "User-Agent": "TURTO-2.2.46-Bootstrap",
    ...
    raise RuntimeError(f"Nelze stáhnout runtime installer 2.2.46.\n{last}")
...
def _repair_runtime() -> None:
    ...
    try:
        ...
        if not PROGRAM_MARKER.is_file():
            raise RuntimeError("Opravený runtime nemá značku verze 2.2.46.")
...
def selftest() -> None:
    assert VERSION == "2.2.46" and RUNTIME_LAYOUT == "30"
    ...
    assert "app_runtime_244.pyw" in CRITICAL_PROGRAM_SHA256
    assert "isokorb_xt_resolver_245.py" in CRITICAL_PROGRAM_SHA256
    assert "isokorb_xt_resolver_246.py" in CRITICAL_PROGRAM_SHA256
