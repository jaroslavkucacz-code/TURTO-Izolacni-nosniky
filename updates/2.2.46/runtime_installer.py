...
"""Verified TURTO 2.2.46 XT bulk-GUI binding installer. Never writes actions.sqlite3."""
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, temp_name = tempfile.mkstemp(prefix=".turto_2246_", dir=path.parent)
    try:
...
def install_runtime(root) -> Path:
    ...
    if not _base_ready(program):
        with tempfile.TemporaryDirectory(prefix="turto_2246_base_") as folder:
            base_installer = Path(folder) / "runtime_installer_244.py"
            base_installer.write_bytes(
                _download(BASE_COMMIT, BASE_PATH, BASE_SHA256, "TURTO-2.2.46-base")
            )
    ...
    payload = {
        name: _download(commit, remote, expected, "TURTO-2.2.46-runtime")
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Ověření runtime 2.2.46 po instalaci selhalo.")
    except Exception:
