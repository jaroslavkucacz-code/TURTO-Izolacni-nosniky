...
"""TURTO Statika 3.0.12 updater – verified updates with backups kept under Zaloha."""
...
# Retained only so cleanup can recognize installations created by older updater versions.
LEGACY_UPDATE_BACKUP = ".update_backup"
BACKUP_RELATIVE = "Zaloha/Aktualizace"
LOG_RELATIVE = "Logy/update_apply.log"
    ...
    *,
    ...
    for attempt in range(max(1, attempts)):
        try:
            ...
            request_headers = {
                "User-Agent": "TURTO-Updater-2.2.15",
...
def _apply_script(version: str, files: list[dict], runtime_layout: str) -> str:
    ...
    return f'''import os, shutil, subprocess, sys, time, traceback
from pathlib import Path
...
try:
    ...
    log("CHYBA při aplikaci aktualizace; zahajuji rollback:\\n" + traceback.format_exc())
...
if executable:
    try:
        subprocess.Popen([executable], cwd=str(dst))
    except Exception:
        log("Aktualizace proběhla, ale restart EXE selhal:\\n" + traceback.format_exc())
