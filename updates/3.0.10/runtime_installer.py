...
"""TURTO 3.0.10: per-row HIT input units; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_10.ok"

PAYLOADS = {'app_runtime.pyw': ('83871f6f21a8d802c7656a795de14c46863f2265',
                     'updates/3.0.10/app_runtime.pyw',
                     '18e5842cd03f1447fb44c2b3c0df80b4648ebba01e44dbfdd1b802fc2c52b159'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.10-runtime", "Cache-Control": "no-cache, no-store"})
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".turto_310_", dir=path.parent)
    try:
...
def install_runtime(root: Path | str) -> Path:
    ...
    if not all((program / n).is_file() for n in BASE_REQUIRED):
        with tempfile.TemporaryDirectory(prefix="turto_310_base_") as folder:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.10 neprošel závěrečnou kontrolou.")
    except Exception:
