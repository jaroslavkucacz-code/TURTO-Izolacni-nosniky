...
class HitInputRow:
    ...
    def recalculate(self, preserve_product: str | None = None) -> None:
        ...
        if not lengths:
            ...
            return
        if self.connection_type.get().strip().upper() == "MVX":
    ...
    def defaults_for_next(self) -> dict[str, str]:
        return {"series": self.series.get(), "connection_type": self.connection_type.get(), "mvx_variant": self.mvx_variant.get(), "mvx_bx": self.mvx_bx.get(), "height": self.height.get(), "cover": self.cover.get(), "concrete": self.concrete.get(), "med_pos": "", "med_neg": "", "ned_pos": "", "ned_neg": "", "ved_pos": "", "ved_neg": "", "load_x": ""}
...
class HitWorkspaceMixin:
    ...
    def _build_hit_tab(self, parent: ttk.Frame) -> None:
        ...
        headers = ("Označení", "Řada", "Typ", "MVX prov.", "bx [mm]", "h [mm]", "cnom", "Beton", "MEd+", "MEd−", "NEd+", "NEd−", "VEd+", "VEd−", "x [mm]", "Navržený výrobek ▼", "Varianty", "Využití", "a max", "Zdroj", "Stav / kontrola", "")
        for column, text in enumerate(headers):
            ttk.Label(self.hit_rows_frame, text=text, style="Card.TLabel", font=("Calibri", 10, "bold"), anchor="w" if column in (0, 15, 20) else "center").grid(row=0, column=column, sticky="ew", padx=2, pady=(0, 5))
            self.hit_rows_frame.columnconfigure(column, weight=1 if column in (0, 15, 20) else 0)
        ttk.Label(parent, text=("Vstupy zatížení se automaticky zamykají podle zvoleného typu: MVX/MVXL = M−, V±; ZVX = pouze V+; ZDX = V±; DD = M±, V±; DVL = M±, V+; DDL = M±, V±; AT = M±, N−, V±; FT = M±, N±, V±; OTX = N±, V+ a vzdálenost x. U MVX se provedení volí přímo v řádku jako Bez / OU / OD. Při OU/OD je bx povinné; číslo za OU/OD je bx [mm] a určuje délku části tahového prutu na straně hlavní desky bx−20 mm. Pole bx nepustí hodnotu nad katalogové maximum a po dokončení vstupu drží standardní rozsah 175–330 mm (HP), resp. 175–290 mm (SP). Good/poor bond MVXL se volí automaticky podle h−cnom. Uzamčená pole mají odlišné podbarvení a při výpočtu se ignorují, ale jejich dříve zadaná hodnota se zachová pro případné přepnutí zpět. ZVX/ZDX mají podle Annexu 3 pevné cnom = 30 mm, proto se u nich volba krytí automaticky nastaví na 30 a uzamkne. Po návratu k jinému typu se obnoví poslední volitelné krytí. Výška v označení HIT je v cm: 200 mm → 20. Cenové doporučení zatím není aktivní: program nejprve hlídá statickou použitelnost a cenu budeme později používat až pro pořadí mezi technicky vyhovujícími variantami."), style="Muted.TLabel", wraplength=1450, justify="left").grid(row=3, column=0, sticky="ew", pady=(8, 0))
    ...
    def copy_hit_results(self) -> None:
        lines = ["Označení\tŘada\tTyp\tMVX prov.\tbx [mm]\th [mm]\tcnom [mm]\tBeton\tMEd+ [kNm/m]\tMEd− [kNm/m]\tNEd+ [kN/m]\tNEd− [kN/m]\tVEd+ [kN/m]\tVEd− [kN/m]\tx [mm]\tNavržený výrobek\tVyužití\ta max [m]\tZdroj\tNRd\tMRd,1\tVRd,1\tMRd,2\tVRd,2"]
        count = 0
