...
def _legacy(raw: str, family: str, size: str, movement: str, detail: str, *, archive_supported: bool, decoder_only: bool) -> dict[str, Any]:
...
def decode_dowel(text: str) -> dict[str, Any] | None:
    ...
    if match: return _legacy(raw,"SLD-Q",match.group(1),"transverse","Historický Schöck Dorn SLD-Q; archivní VRd je integrováno v TURTO.",archive_supported=True,decoder_only=False)
    match = _SLD.search(upper)
    if match: return _legacy(raw,"SLD",match.group(1),"axial","Historický Schöck Dorn SLD; archivní VRd je integrováno v TURTO.",archive_supported=True,decoder_only=False)
    match = _LD.search(upper)
    if match:
        q=bool(match.group("q")); sleeve=match.group("sleeve").upper(); material=match.group("material").upper().replace("ZN","Zn")
        ...
        return _legacy(raw,"LD-Q" if q else "LD",match.group("size"),"transverse" if q else "axial",f"Historický Schöck Dorn LD: {sleeve_text}, {material_text}; archivní VRd je integrováno v TURTO.",archive_supported=True,decoder_only=False)
    match = _PART.search(upper)
    if match:
        q=bool(match.group("q")); part=match.group("part").upper().replace("ZN","Zn")
        return _legacy(raw,"LD-Q" if q else "LD",match.group("size"),"transverse" if q else "axial",f"Historické komponentové značení Schöck Dorn LD – Part {part}. Samostatná komponenta neurčuje celý komplet.",archive_supported=False,decoder_only=True)
    return _ORIGINAL_DECODE(raw)

def catalog_summary() -> str:
    return _ORIGINAL_SUMMARY() + " • Archiv Schöck Dorn: SLD/SLD-Q 40–150 a kompletní staré LD/LD-Q s tabulkovým VRd"
