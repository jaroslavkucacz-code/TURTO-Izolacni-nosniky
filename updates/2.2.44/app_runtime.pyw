...
"""TURTO 2.2.44 – maintenance wrapper over the verified 2.2.43 XT runtime."""
...
BASE_RUNTIME = PROGRAM / "app_runtime_243.pyw"
if not BASE_RUNTIME.is_file():
    raise RuntimeError(f"Chybí ověřený runtime základ 2.2.43: {BASE_RUNTIME}")

_namespace = runpy.run_path(str(BASE_RUNTIME), run_name="turto_runtime_243")
...
def selftest() -> None:
    assert APP_VERSION == "2.2.44"
    assert BASE_RUNTIME.name == "app_runtime_243.pyw"
