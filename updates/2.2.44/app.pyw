...
"""TURTO 2.2.44 – stabilized release chain over the verified 2.2.43 XT runtime."""
import hashlib
import importlib
...
INSTALLER_COMMIT = "35211cdb9cfa06a06bdf19c4fdeb093f255444b5"
INSTALLER_SHA256 = "c48e5209e605dce8c46ed571754bc3bf10670460f3d706f792d1f70e7074bb14"
RUNTIME_LAYOUT = "28"

ROOT = Path(__file__).resolve().parent
PROGRAM = ROOT / "Program"
ROOT_MARKER = ROOT / ".turto_runtime_current.ok"
PROGRAM_MARKER = PROGRAM / ".turto_runtime_2_2_44.ok"
STARTUP_LOG = ROOT / "Logy" / "startup.log"

CRITICAL_PROGRAM_SHA256 = {
    "app_runtime.pyw": "815ba2950af3145552c0d40ed2771bd3448c0c00d6feab580c321ee9410b4627",
    "app_runtime_243.pyw": "fd527e691770a5547f387e1cb47c535aae06df8dc063f07917bdcfd88a07f752",
    "app_runtime_242.pyw": "5b73f92db8500c53080d92750752d8b0df35b16b1f557b581804fd094dbc8745",
...
def _download_installer() -> Path:
    url = f"https://raw.githubusercontent.com/{REPOSITORY}/{INSTALLER_COMMIT}/updates/2.2.44/runtime_installer.py"
    folder = Path(tempfile.mkdtemp(prefix="turto_2244_boot_"))
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(
                ...
                headers={"User-Agent":"TURTO-2.2.44-Bootstrap","Cache-Control":"no-cache, no-store","Pragma":"no-cache"},
            )
    ...
    raise RuntimeError(f"Nelze stáhnout runtime installer 2.2.44.\n{last}")
...
def _repair_runtime() -> None:
    ...
    try:
        ...
        if not PROGRAM_MARKER.is_file():
            raise RuntimeError("Opravený runtime nemá značku verze 2.2.44.")
...
def selftest() -> None:
    assert VERSION == "2.2.44" and RUNTIME_LAYOUT == "28"
    assert len(INSTALLER_SHA256) == 64 and all(len(v) == 64 for v in CRITICAL_PROGRAM_SHA256.values())
    assert "app_runtime_243.pyw" in CRITICAL_PROGRAM_SHA256
    assert "isokorb_xt_parser_243.py" in CRITICAL_PROGRAM_SHA256
    assert "shear_cret_sync_242.py" in CRITICAL_PROGRAM_SHA256
