...
DATA_FILENAME = "hit_all_2023_v4.json.gz.b64"
...
class Candidate:
    ...
    def physical_length_mm(self) -> int:
        ...
        return {100: 1000, 50: 500, 33: 330, 25: 250}.get(self.length_code, self.length_code)
...
def _parse_zvx_pages(pdf) -> list[dict[str, Any]]:
    ...
    width_mm = {"100": 1000, "050": 500, "033": 330, "025": 250}
...
def build_database_from_pdf(pdf_path: Path, out_path: Path):
    ...
    with pdfplumber.open(str(pdf_path)) as pdf:
        ...
        if len(mvx_sections) < 190:
            raise RuntimeError(f"Z Annexů 1/2 se podařilo načíst jen {len(mvx_sections)} tabulek MVX; očekáváno přibližně 200.")
    ...
    payload = {
        "schema_version": 4,
        "catalog_id": "leviat_hit_hp_sp_07_23_all_v4",
...
class HitDatabase:
    def __init__(self, path: Path):
        ...
        # Annex 2 gives the OD/WD design values in common tables headed OD.
    ...
    def proposal_candidates(
        ...
        primary, primary_error = self.directional_candidates(
            typ, series, height, cover, concrete, actions, lengths, include_offsets, load_distance_x
        )
        info: dict[str, Any] = {
            "preferred_type": typ,
        ...
        alternative, alternative_error = self.directional_candidates(
            alternative_type, series, height, cover, concrete, actions, lengths, include_offsets, load_distance_x
        )
