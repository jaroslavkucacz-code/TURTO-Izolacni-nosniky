...
def mvx_bx_limits(series: str) -> tuple[int, int]:
...
class HitInputRow:
    def __init__(self, owner: "HitWorkspaceMixin", row_no: int, defaults: dict[str, Any] | None = None):
        ...
        self.detail = tk.StringVar(value="čeká na zatížení")
    ...
    def _validate_mvx_bx_keystroke(self, proposed: str) -> bool:
    ...
    def _apply_mvx_variant_state(self) -> None:
        typ = self.connection_type.get().strip().upper()
        is_mvx = typ == "MVX"
        if is_mvx:
...
class HitWorkspaceMixin:
    ...
    def _build_hit_tab(self, parent: ttk.Frame) -> None:
        ...
        lengths = ttk.Frame(toolbar, style="App.TFrame")
        lengths.grid(row=0, column=5, columnspan=5, sticky="w")
        ttk.Label(lengths, text="Povolené délky").pack(side="left")
        for label, variable in (("1000 mm", self.hit_l100_var), ("500 mm", self.hit_l050_var), ("330 mm", self.hit_l033_var), ("250 mm", self.hit_l025_var)):
            ttk.Checkbutton(lengths, text=label, variable=variable, command=self._hit_options_changed).pack(side="left", padx=(7, 0))
        ...
        headers = ("Označení", "Řada", "Typ", "MVX prov.", "bx [mm]", "h [mm]", "L/B požad. [mm]", "cnom", "Beton", "MEd+", "MEd−", "NEd+", "NEd−", "VEd+", "VEd−", "HEd∥", "HEd⊥", "x [mm]", "Navržený výrobek ▼", "Varianty", "Využití", "a max", "Zdroj", "Stav / kontrola", "")
        for column, text in enumerate(headers):
            ttk.Label(self.hit_rows_frame, text=text, style="Card.TLabel", font=("Calibri", 10, "bold"), anchor="w" if column in (0, 18, 23) else "center").grid(row=0, column=column, sticky="ew", padx=2, pady=(0, 5))
            self.hit_rows_frame.columnconfigure(column, weight=1 if column in (0, 18, 23) else 0)
        ttk.Label(parent, text=("Vstupy se zamykají podle typu. HIT-HT používá pouze HEd∥/HEd⊥ v kN/prvek; MEd/NEd/VEd jsou u HT uzamčené. "
    ...
    def open_hit_schedule(self) -> None:
    ...
    def copy_hit_results(self) -> None:
        lines = ["Označení\tŘada\tTyp\tMVX prov.\tbx [mm]\th [mm]\tcnom [mm]\tBeton\tMEd+ [kNm/m]\tMEd− [kNm/m]\tNEd+ [kN/m]\tNEd− [kN/m]\tVEd+ [kN/m]\tVEd− [kN/m]\tHEd∥ [kN/prvek]\tHEd⊥ [kN/prvek]\tx [mm]\tNavržený výrobek\tVyužití\ta max [m]\tZdroj\tNRd\tMRd/HRd∥,1\tVRd/HRd⊥,1\tMRd,2\tVRd,2\tL/B požadované [mm]\tVýkaz – původní text"]
        count = 0
        for row in self.hit_rows:
            candidate = row.selected_candidate
            if candidate is None: continue
            effective = row.effective_action_texts()
            lines.append("\t".join([row.name.get(), row.series.get(), candidate.connection_type, (row.mvx_variant.get() if candidate.connection_type == "MVX" else ""), (row.mvx_bx.get() if candidate.connection_type == "MVX" and row.mvx_variant.get().strip().upper() in {"OU", "OD"} else ""), row.height.get(), row.cover.get(), row.concrete.get(), effective["m_pos"], effective["m_neg"], effective["n_pos"], effective["n_neg"], effective["v_pos"], effective["v_neg"], effective.get("h_parallel", ""), effective.get("h_perp", ""), (fmt(candidate.load_distance_x,0) if candidate.load_distance_x else ""), candidate.designation, ("" if candidate.spacing_max > 0 else fmt(candidate.utilization * 100.0) + " %"), (fmt(candidate.spacing_max,3) if candidate.spacing_max > 0 else ""), str(candidate.page), fmt(candidate.nrd), fmt(candidate.m1), fmt(candidate.v1), fmt(candidate.m2), fmt(candidate.v2), row.required_length.get(), row.import_source_text.replace("\t", " ").replace("\n", " ")]))
            count += 1
        if not count:
