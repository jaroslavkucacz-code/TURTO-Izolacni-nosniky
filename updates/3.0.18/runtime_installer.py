...
"""TURTO 3.0.18: restored original catalogues; existing customer files are read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_18.ok"

PAYLOADS = {'app_runtime.pyw': ('51a3cf7b1a464219241b23114d7b85295edf56f3',
                     'updates/3.0.18/app_runtime.pyw',
                     'fe58a506bd28323bb58b99302b407ef44c4282a6d2a8e66939920e7cee582a6c'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.18-runtime", "Cache-Control": "no-cache, no-store"})
...
def install_runtime(root: Path | str) -> Path:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.18 neprošel závěrečnou kontrolou.")
    except Exception:
