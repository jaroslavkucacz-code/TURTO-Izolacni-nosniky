...
"""TURTO 3.0.8: T-QP-VV to HIT substitution; customer data is read-only."""
import hashlib
import os
...
MARKER = ".turto_runtime_3_0_8.ok"

PAYLOADS = {'app_runtime.pyw': ('f005b4246949fb308e48e890c48531e8daaf1961',
                     'updates/3.0.8/app_runtime.pyw',
                     '96176ca18a2c3ddba210023b5306eed9b8574325eed2f0de8c951ffe130c3639'),
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url + f"?turto={time.time_ns()}", headers={
                "User-Agent": "TURTO-3.0.8-runtime", "Cache-Control": "no-cache, no-store"})
...
def install_runtime(root: Path | str) -> Path:
    ...
    try:
        ...
        if not _revision_ok(program):
            raise RuntimeError("Runtime TURTO 3.0.8 neprošel závěrečnou kontrolou.")
    except Exception:
...
def selftest() -> None:
    assert VERSION == "3.0.8" and RUNTIME_LAYOUT == "39"
