...
class HitWorkspaceMixin:
    ...
    def _build_hit_tab(self, parent: ttk.Frame) -> None:
        ...
        headers = ("Označení", "Řada", "Typ", "MVX prov.", "bx [mm]", "h [mm]", "L požad. [mm]", "cnom", "Beton", "MEd+", "MEd−", "NEd+", "NEd−", "VEd+", "VEd−", "x [mm]", "Navržený výrobek ▼", "Varianty", "Využití", "a max", "Zdroj", "Stav / kontrola", "")
        for column, text in enumerate(headers):
            ttk.Label(self.hit_rows_frame, text=text, style="Card.TLabel", font=("Calibri", 10, "bold"), anchor="w" if column in (0, 16, 21) else "center").grid(row=0, column=column, sticky="ew", padx=2, pady=(0, 5))
            self.hit_rows_frame.columnconfigure(column, weight=1 if column in (0, 16, 21) else 0)
    ...
    def copy_hit_results(self) -> None:
        lines = ["Označení\tŘada\tTyp\tMVX prov.\tbx [mm]\th [mm]\tcnom [mm]\tBeton\tMEd+ [kNm/m]\tMEd− [kNm/m]\tNEd+ [kN/m]\tNEd− [kN/m]\tVEd+ [kN/m]\tVEd− [kN/m]\tx [mm]\tNavržený výrobek\tVyužití\ta max [m]\tZdroj\tNRd\tMRd,1\tVRd,1\tMRd,2\tVRd,2\tL požadované [mm]\tVýkaz – původní text"]
        count = 0
