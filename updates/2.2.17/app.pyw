...
"""TURTO 2.2.17 bootstrap – flattened HIT runtime and one-shot housekeeping."""
...
INSTALLER_COMMIT = "7981844861c7485f74795475aec41f725c803520"
INSTALLER_SHA256 = "2396cd9c5fd0e01ccd8d1f618e2905a113b415d82b73ef32d508189bacfdf8bd"
RUNTIME_LAYOUT = "10"

ROOT = Path(__file__).resolve().parent
PROGRAM = ROOT / "Program"
MARKER = ROOT / ".turto_runtime_current.ok"
PROGRAM_REVISION = PROGRAM / ".turto_runtime_2_2_17.ok"
CLEANUP_REVISION = ROOT / ".turto_cleanup_2_2_17.ok"
LOG_DIR = ROOT / "Logy"
...
def _download_installer() -> Path:
    url = (
        ...
        "updates/2.2.17/runtime_installer.py"
    )
    last = None
    ...
    for attempt in range(4):
        try:
            request = urllib.request.Request(
                ...
                headers={
                    "User-Agent": "TURTO-2.2.17-Bootstrap",
...
def _install_runtime() -> None:
    ...
    try:
        ...
        if not PROGRAM_REVISION.is_file():
            raise RuntimeError("Runtime revize 2.2.17 nebyla vytvořena.")
...
def _cleanup_needed() -> bool:
    ...
    if (ROOT / "Archiv" / "Runtime_pred_2.2.13").exists():
        return True
    return any((PROGRAM / name).is_file() for name in OBSOLETE_PROGRAM_FILES)
...
def _run_cleanup() -> None:
    ...
    try:
        ...
        completed = False
    try:
        from cleanup_stage6 import cleanup_stage6
        cleanup_stage6(ROOT, PROGRAM, LOG_DIR, CLEANUP_LOG)
    except Exception:
        completed = False
