...
"""TURTO 2.2.9 – multi-selection PDF scope for AKCE reports."""
...
def _normalize_scope_ids(scope_ids: str | Iterable[str] | None) -> tuple[str, ...]:
    if scope_ids is None:
        return ("all",)
    if isinstance(scope_ids, str):
        values = [scope_ids]
    else:
        values = [str(value) for value in scope_ids]
    out: list[str] = []
    for value in values:
        key = str(value or "").strip()
        if key in SCOPE_BY_ID and key not in out:
            out.append(key)
    return tuple(out or ["all"])


def _section_keys_for_scopes(scope_ids: str | Iterable[str] | None) -> tuple[tuple[str, str], ...]:
    ids = _normalize_scope_ids(scope_ids)
    all_keys = (
        ("thermal_breaks", "decoder"),
        ("thermal_breaks", "design"),
        ("thermal_breaks", "substitution"),
        ("shear_dowels", "decoder"),
        ("shear_dowels", "design"),
        ("shear_dowels", "substitution"),
    )
    if "all" in ids:
        return all_keys
...
def rows_for_scopes(owner: Any, scope_ids: str | Iterable[str] | None) -> list[dict[str, Any]]:
    sections = collect_report_sections(owner)
    rows: list[dict[str, Any]] = []
    for key in _section_keys_for_scopes(scope_ids):
...
def _selection_label(scope_ids: tuple[str, ...]) -> str:
    if "all" in scope_ids:
        return SCOPE_BY_ID["all"].label
    labels = [SCOPE_BY_ID[value].label for value in scope_ids]
    if len(labels) == 1:
        return labels[0]
    return f"Výběr ({len(labels)} položky): " + " • ".join(labels)
...
class ScopeDialog(tk.Toplevel):
    def __init__(self, owner: Any) -> None:
        ...
        self.variables = {
            scope.id: tk.BooleanVar(master=self, value=(scope.id == "all"))
            for scope in SCOPES
        }
        ...
        ttk.Label(
            outer,
            text="Zaškrtněte jednu nebo více částí. Překrývající se volby se do PDF vloží jen jednou.",
            style="Muted.TLabel",
            wraplength=640,
            justify="left",
        ...
        for index, scope in enumerate(SCOPES):
            ...
            check = ttk.Checkbutton(
                card,
                text=scope.label,
                variable=self.variables[scope.id],
                command=lambda scope_id=scope.id: self._on_toggle(scope_id),
            )
            check.pack(anchor="w", pady=3)
            self.checkbuttons[scope.id] = check

        tools = ttk.Frame(outer, style="App.TFrame")
        tools.pack(fill="x", pady=(10, 0))
        ttk.Button(tools, text="Vše", command=self._select_all).pack(side="left")
        ttk.Button(tools, text="Nic", command=self._select_none).pack(side="left", padx=(7, 0))
    ...
    def _on_toggle(self, scope_id: str) -> None:
        if scope_id == "all" and self.variables["all"].get():
            for key, variable in self.variables.items():
                if key != "all":
                    variable.set(False)
            return
        if scope_id != "all" and self.variables[scope_id].get():
            self.variables["all"].set(False)
            if scope_id.startswith("iso.") and scope_id != "iso.all":
                self.variables["iso.all"].set(False)
            elif scope_id == "iso.all":
                for key in ("iso.decoder", "iso.design", "iso.substitution"):
                    self.variables[key].set(False)
            if scope_id.startswith("shear.") and scope_id != "shear.all":
                self.variables["shear.all"].set(False)
            elif scope_id == "shear.all":
                for key in ("shear.decoder", "shear.design", "shear.substitution"):
                    self.variables[key].set(False)

    def _select_all(self) -> None:
        for key, variable in self.variables.items():
            variable.set(key == "all")
...
def export_pdf(owner: Any, scope_ids: str | Iterable[str]) -> None:
    selected = _normalize_scope_ids(scope_ids)
    ...
    try:
        ...
        return
    if not rows:
        messagebox.showinfo("Export PDF", f"Ve zvoleném rozsahu „{label}“ není žádný řádek k exportu.", parent=owner)
        return
    try:
    ...
    if "all" in selected:
        ...
        suffix = re.sub(r"[^A-Za-z0-9_-]+", "_", selected[0].replace(".", "_"))
    else:
        suffix = f"Vyber_{len(selected)}"
    path = filedialog.asksaveasfilename(
        parent=owner,
        title=f"Uložit PDF – {label}",
    ...
    try:
        ...
        report_title = action if "all" in selected else f"{action} — {label}"
...
def selftest() -> None:
    assert len(SCOPES) == 9
    assert _normalize_scope_ids(("iso.decoder", "shear.design")) == ("iso.decoder", "shear.design")
    assert _section_keys_for_scopes(("iso.all", "iso.decoder")) == (
        ("thermal_breaks", "decoder"),
        ("thermal_breaks", "design"),
        ("thermal_breaks", "substitution"),
    )
    assert len(_section_keys_for_scopes(("iso.decoder", "shear.design"))) == 2
    assert len(_section_keys_for_scopes(("all", "iso.decoder"))) == 6
