...
"""TURTO 2.2.4 – unified tables and cross-domain UI consistency."""
...
def install(app_base: Any) -> None:
    ...
    if getattr(cls, "_turto_platform_workspace_224_installed", False): return
    _platform_base.install(app_base); install_help(app_base)
    cls = app_base.ThermalConnectorApp; install_methods(cls); _platform_base.export_action_pdf = _action_report.export_action_pdf
    original_body = cls._build_body

    def body(self) -> None:
        original_body(self); init_shear_workspace(self); _update_branding(self)
        frame = getattr(self, "product_domain_tab_by_id", {}).get("shear_dowels")
        if frame is not None:
            for child in list(frame.winfo_children()):
                try: child.destroy()
                except Exception: pass
            build_shear_workspace(self, frame); attach_shear_autocomplete(self)
        _hide_inactive_domains(self); compact_inline_help(self)
        install_table_controls(self); apply_ui_consistency(self)
        try:
            self.base_window_title = f"TURTO {WORKSPACE_VERSION} – Technické prvky"; self._update_project_title()
        except Exception: pass

    cls._build_body = body; cls._turto_platform_workspace_224_installed = True
