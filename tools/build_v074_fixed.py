...
ICON_SOURCE = ROOT / "assets" / "app_icon64.png.b64"
...
def patch_app(text: str) -> str:
    text = replace_once(text, 'APP_VERSION = "0.7.3"', 'APP_VERSION = "0.7.4"', "version")
    text = replace_once(
        text,
    ...
    text = replace_once(text, old_icon, new_icon, "icon loader")
    text = replace_once(
        text,
...
def main() -> None:
    ...
    ui_path = OUT / "ui_utils.py"
    ui_text = ui_path.read_text(encoding="utf-8")
    if "maximize_window_on_pointer_monitor" not in ui_text:
        ui_path.write_text(ui_text.rstrip() + MAXIMIZE_HELPER + "\n", encoding="utf-8")
    ...
    assert "MonitorFromPoint" in ui_text
    ...
    (OUT / "RELEASE_NOTES.txt").write_text(
        ...
        "- Přidána průhledná ikona TURTO podle dodaného firemního loga.\n"
        "- Ikona se používá v titulkové liště, na hlavním panelu Windows i v hlavičce aplikace.\n"
    ...
    manifest = {
        "version": "0.7.4",
        "notes": "TURTO se při startu vždy maximalizuje na monitoru pod kurzorem a používá nové průhledné firemní logo jako ikonu aplikace.",
